...
- **Fontes dos recebidos por loja (2026-09-12) — regra ÚNICA para Receitas, DRE, DRE Comparativo e Visão Geral:** tabela `fin_revenue_settings` (`tenant_id` PK, `sources text[]` ⊂ `orders|stone|pix|ifood|cash|manual`). Sem linha = `orders + manual` (comportamento antigo). Helper compartilhado `src/lib/revenueSources.ts` (`fetchRevenueSources`, `loadRevenueExtras`, `applyRevenueSources`, `fetchPixRecebidos`, `fetchStoneSales`).
  - `orders` = `auto_sale` (na DRE, as linhas por destino); `manual` = `fin_cash_flow` manual; `stone` = `fin_cash_flow` `origin='stone_sale'` (bruto liquidado, dia do pagamento; só existe com `post_to_ledger`); `pix` = `fin_bank_statement_imports` `source='inter'`, crédito, `raw->>tipoTransacao='PIX'` — **inclui** a transferência da Conta Stone da própria empresa (`match_kind='internal_transfer'`), que é o Pix vendido na maquininha. Pegadinha: aporte de sócio por Pix também entra. **Leitura pela RPC `fin_pix_recebidos(p_tenant, p_start, p_end)`** (security definer, checa `auth_is_member_of`): `fin_bank_statement_imports` não tem GRANT para `authenticated` de propósito — select direto do front dá `permission denied`.
  - Visão Geral: `entradas` do saldo exclui sempre `auto_sale`/`manual`/`stone_sale` (ou já são receita, ou a loja escolheu não contá-los). Ticket médio continua sendo dos pedidos (auto_sale ÷ nº de pedidos).
  - Configurado na aba Receitas (botão **Fontes** no aviso azul), escrita pela ação `set_revenue_sources` do `financial-write`; leitura direta (policy `auth_is_member_of`).
  - **`cash` (2026-09-22) — vendas em dinheiro do PDV/totem:** dinheiro em espécie não passa por banco nem maquininha, então em loja que conta só o que entra na conta ele não tinha fonte nenhuma e sumia de Receitas e da DRE (achado no go-live de Paranaguá: 21/09, R$ 247,60 em 3 vendas fora do financeiro). Soma os `auto_sale` cujo `reference_id` é um `payments` de método `type='cash'`, pela data da venda, pela RPC `fin_dinheiro_recebidos(p_tenant, p_start, p_end)` (security definer, `auth_is_member_of`; o join `fin_cash_flow → payments → payment_methods` não deve ser montado no front). **Sem efeito quando `orders` está ligado** — ali o `auto_sale` já traz o dinheiro junto (guarda em `applyRevenueSources` e em cada tela). Estorno não é descontado aqui: ele já entra como despesa própria, igual ao `auto_sale`.
  - **El Patron Paranaguá = `stone + pix + ifood + cash`** (desde 2026-09-22): o que conta é o dinheiro que entrou (conta, maquininha ou gaveta), não o total lançado no PDV.
- **A conciliação bancária (extrato) NÃO alimenta Receitas.** `fin_bank_statement_imports` só marca `reconciled=true` na linha do extrato (`useConciliacao.reconcile`); não cria receita, não baixa conta a pagar, não liquida recebível, não lança em `fin_cash_flow`. É conferência, não lançamento. Em 2026-09-05 não havia **nenhuma** linha de extrato importada em nenhuma loja.
...
- **~~Limitação conhecida~~ CORRIGIDA (2026-09-13):** o caixa filtrava as compras por `payment_status in ('paid','partial')` **por `purchase_date`**. Em set/2026 (Paranaguá) a DRE mostrava CMV de R$ 1.127,95 enquanto saíram R$ 3.528,74 em compras (9 contas, compras de agosto pagas em setembro). Agora `fetchComprasPeriodo(tenant, ini, fim, mode)`: **caixa** = contas a pagar `reference_type='purchase'` com `paid_date` no mês (`paid_amount`) + compra paga sem conta a pagar (à vista) pela data da compra; **competência** = compras com `purchase_date` no mês, pagas ou não. Cada compra leva um `peso` (pago ÷ total, máx. 1) aplicado a todos os itens no `splitComprasDRE`.
- **Drill-down do CMV (2026-09-13):** lia o consumo por ficha técnica (`order_items.unit_cost`) e abria vazio. Agora `fetchComprasLinhas` devolve item a item (fornecedor, NF, qtd, categoria = do item ou do insumo), com o mesmo conjunto/peso da linha; agrupa por categoria ou por fornecedor.
- **Folha no caixa (2026-09-13):** era `status='paid'` + `reference_month` = mês da DRE; a folha de agosto paga em setembro nunca aparecia. Agora `paid_date` no mês. Competência segue por `reference_month`.
...
- **Período a importar (2026-09-12):** topo da Conciliação tem De/Até + "Importar período" (Inter e Stone juntos); o botão ↻ ao lado continua "desde o último sync". `inter-bank › sync` aceita `date_from`/`date_to` (até 1 ano; ignora o `sync_from` da config quando o período é explícito) e **só grava `last_sync_at` se o período terminar hoje** — senão o próximo sync incremental pularia dias. A Stone é chamada em blocos de 31 dias (`import_range`), com Até limitado a ontem. O `InterSyncPanel` também ganhou De/Até.
- **Sem cron (decisão do usuário, 2026-09-10):** o extrato é buscado quando alguém abre Financeiro › Conciliação (`ConciliacaoTab` chama `inter-bank › sync` e `stone-conciliation › sync` ao montar e ao trocar de loja) ou clica em **Atualizar bancos**. O cron `inter-bank-sync` que chegou a ir ao ar é removido pela migration `20260910150000_conciliacao_sem_cron.sql`. `auto_sync=false` na config faz o banco ser pulado ao abrir a tela. Erro de sync fica em `last_sync_error` e aparece no painel.
...
**Estado:** a edge `stone-conciliation` nunca funcionou. Buscava a loja em `users.tenant_id`, coluna que não existe mais, e toda chamada voltava 400 "Tenant not found". O parser também não seguia o layout: dividia valores por 100, cortava errado as datas `aaaammddHHmmss` e gerava id aleatório, o que duplicava as linhas. Nenhuma loja tinha config. O código foi reescrito (edge, `StoneConfigModal`, `StoneImportPanel`). **No ar desde 2026-09-10:** edge `stone-conciliation` v12 (verify_jwt=false) e as migrations `20260910140000_stone_conciliation_v2.sql` e `20260910150000_conciliacao_sem_cron.sql` aplicadas. Front pendente de push. Smoke test na edge publicada: `get_config` → `config: null`; `sync` → `not_configured` (nenhuma loja configurou ainda).
...
- **Sem cron:** ao abrir a Conciliação ou no botão **Atualizar bancos**, `sync` importa ontem e os dias sem sucesso dos últimos 3 (os que já deram certo não são baixados de novo). Para períodos maiores, o painel da Stone tem "Importar período" (até 31 dias).
...
- **Desde 2026-09-22 a loja pode ter MAIS DE UMA maquininha ao mesmo tempo** (`fin_card_providers`: uma linha por maquininha com conta do repasse, texto no extrato e modo do Pix; `fn_card_providers(p_tenant)` com fallback para a configuração antiga). `fn_match_card_deposits` (Stone) e `fn_match_mp_payouts` (Mercado Pago) leem cada uma a SUA linha — antes o gate era `fn_money_flow.card_provider`, que é um só: em Paranaguá, escolher o Mercado Pago em 21/09 parou de casar os repasses da Stone (26 grupos represados, religados em 22/09). `fn_money_flow` continua com a mesma assinatura: `card_provider`/`card_deposit_*` = maquininha principal (Stone > Mercado Pago > outra) e `card_pix_mode='transfer'` se QUALQUER maquininha transferir o Pix. As vendas nunca precisaram disso: todo conector grava `fin_cash_flow` origin `stone_sale`. Escrita pela ação `set_card_providers` do `financial-write` (substitui a lista inteira); tela em Conciliação › ⚙ › Como o dinheiro entra (checkbox por maquininha).
...
**Vínculo item → insumo no RECEBIMENTO (2026-09-11, pedido do dono):**
- Motivo: o estoque só entra no recebimento (`fin_purchases.stock_applied_at`), mas o vínculo com o insumo só era feito na importação da nota. A nota importada automaticamente pela conciliação chegava sem vínculo e o item nunca entrava no estoque; e compra paga/recebida não pode ser editada.
- Migration `20260911170000_purchase_items_supplier_code.sql`: `fin_purchase_items.supplier_code` e `ean` (backfill pelas notas: 25 de 28 itens). `purchase-write` grava os dois; `fiscal-inbound` (`import_purchase`) envia `it.codigo` e `it.ean`.
- `purchase-confirm-delivery`: ação `receipt_context` devolve os insumos e uma sugestão por item (o que já está na compra → memorizado em `fiscal_inbound_item_links` pelo CNPJ do fornecedor + código, ou pelo EAN → última compra com a mesma descrição). Na confirmação, `received_items[].ingredient_id` / `units_per_package` gravam o vínculo no item, o item recém-vinculado entra **inteiro** no estoque (mesmo em compra antiga) e o vínculo é memorizado (vazio = esquece). Em compra antiga, item que já tinha insumo não troca.
- Tela (`DetalhePurchaseModal`): cada item do "Confirmar recebimento" tem o seletor de insumo e o fator "1 cx = N un", já preenchidos pela sugestão; aviso de quantos itens não entram no estoque. O front só manda o vínculo quando a lista de insumos carregou, para não apagar vínculos memorizados.

## 9f. Classificação por ITEM — CMV × despesa (2026-09-12)

**Regra (dono):** a classificação mora no **item** comprado (fornecedor + código do produto; sem código, descrição normalizada), **nunca** no total pago ao CNPJ — a mesma nota traz bebida (CMV) e produto de limpeza (despesa). **Embalagem/descartável de delivery é CMV.** Serviço (NFS-e, contador, sistema) continua classificado pela conta inteira.

- **CMV com categoria (2026-09-14, migration `20260914140000_item_classificacao_cmv_categoria.sql`):** `fin_item_classifications.merchandise_category_id` = categoria de mercadoria (`fin_merchandise_categories`, a mesma lista dos insumos). `fn_item_classify(..., p_merchandise_category_id)` (5º parâmetro opcional; sem ele o item mantém a categoria que tinha) grava a categoria nos `fin_purchase_items` já lançados do item (sem insumo) — é de lá que `comprasDRE` abre o CMV por categoria na DRE. O trigger `fn_item_registry_from_purchase_item` preenche a categoria de item de compra novo sem categoria. Item com insumo usa a categoria do insumo. Tela: seletor com busca "CMV…" por linha, filtro "CMV sem categoria", categoria na barra de lote.
- **Base:** `fin_item_classifications` — `classe` `'cmv' | 'despesa' | null` (pendente = CMV na DRE), `dre_category_id` (obrigatória em despesa), `ingredient_id` (item de estoque = sempre CMV), sugestão por NCM/descrição (`suggested_classe`, `suggested_dre_category_id`, `suggestion_reason`), `auto_classified` (CMV automático "a conferir"). Chaves: `fn_item_supplier_key` (CNPJ ou `n:`+nome) e `fn_item_key` (`c:`+código — inclusive o código entre parênteses no fim da descrição de compra antiga — ou `d:`+descrição).
- **Entrada sozinha:** trigger `trg_item_registry_from_doc` (NF-e, `after insert or update of itens`) registra os itens quando o XML chega; trigger `trg_item_registry_from_purchase_item` (`before insert` em `fin_purchase_items`, **qualquer caminho**: nota, tela, assistente, cupom) registra e aplica — despesa → `fin_purchase_items.dre_category_id`, que o split da DRE (§9d, `comprasDRE.ts`) tira do CMV. Os dois triggers engolem erro (nunca travam a sincronização nem o lançamento).
- **Classificar:** `fn_item_classify(p_tenant, p_ids[], p_classe, p_dre_category_id)` (só admin; valida categoria de despesa) reaplica nas compras **já lançadas** do item (itens sem insumo). Tela **Financeiro › Classificação de Itens** (`ItensClassificacaoTab`): pendentes / automáticos / CMV / despesa, filtro por fornecedor, lote (CMV, despesa + categoria, aceitar sugestões, voltar a pendente).
