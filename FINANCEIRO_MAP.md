...
- **Fontes dos recebidos por loja (2026-09-12):** tabela `fin_revenue_settings` (`tenant_id` PK, `sources text[]` ⊂ `orders|stone|manual`). Sem linha = `orders + manual` (comportamento antigo). `stone` = `fin_cash_flow` `origin='stone_sale'` (bruto liquidado, datado no dia do pagamento da Stone; só existe com `post_to_ledger` ligado). Configurado na própria aba (botão **Fontes** no aviso azul), escrita pela ação `set_revenue_sources` do `financial-write`; leitura direta (policy `auth_is_member_of`). **El Patron Paranaguá = só `stone`**: o que conta é o dinheiro que entrou, não o que foi lançado no PDV. Afeta só a aba Receitas — DRE/Visão Geral continuam com a base própria (`auto_sale` + `stone_sale`).
