...
- **Fontes dos recebidos por loja (2026-09-12) — regra ÚNICA para Receitas, DRE, DRE Comparativo e Visão Geral:** tabela `fin_revenue_settings` (`tenant_id` PK, `sources text[]` ⊂ `orders|stone|pix|manual`). Sem linha = `orders + manual` (comportamento antigo). Helper compartilhado `src/lib/revenueSources.ts` (`fetchRevenueSources`, `loadRevenueExtras`, `applyRevenueSources`, `fetchPixRecebidos`, `fetchStoneSales`).
  ...
  - **El Patron Paranaguá = `stone + pix`**: o que conta é o dinheiro que entrou, não o lançado no PDV. iFood (transferência + domicílio no Inter) ainda NÃO é fonte.
