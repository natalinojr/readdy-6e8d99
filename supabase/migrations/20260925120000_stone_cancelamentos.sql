-- Cancelamento de venda Stone: a Stone desconta o líquido da venda cancelada do repasse (Billing.ChargedAmount).
-- A edge stone-conciliation grava isso como linha débito raw.kind='cancellation' (returned = bruto devolvido,
-- charged = valor descontado, pilha = pilha do pagamento de onde saiu). Aqui o cancelamento abate bruto/taxa/líquido
-- do repasse (fn_stone_repasses, fn_match_card_deposits) e a receita por competência (fn_dre_competencia_cartoes).
...
AS $function$
  ...
  st as (
    select transaction_date as d,
           ...
           -- cancelamento: raw.gross/raw.net vêm negativos (bruto devolvido / valor descontado do repasse)
     ...
     where tenant_id = p_tenant and source = 'stone'
       and ((raw->>'kind' = 'installment' and transaction_type = 'credit') or raw->>'kind' = 'cancellation')
       and status <> 'ignored'
    ...
    for g in
      with st as (
        select transaction_date as d,
          ...
          from fin_bank_statement_imports
         where tenant_id = p_tenant and source = 'stone' and ((raw->>'kind' = 'installment' and transaction_type = 'credit') or raw->>'kind' = 'cancellation')
           and status <> 'ignored'
  ...
  if v_stone_ativo then
    select coalesce(sum(coalesce((raw->>'gross')::numeric, amount)), 0),
     ...
     where tenant_id = p_tenant and source = 'stone' and status <> 'ignored'
       -- cancelamento abate a venda (raw.gross/raw.net negativos, capture_date = data da venda original)
       and ((raw->>'kind' = 'installment' and transaction_type = 'credit') or raw->>'kind' = 'cancellation')
