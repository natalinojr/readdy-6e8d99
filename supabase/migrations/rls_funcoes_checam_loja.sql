-- 2026-09-25 — Auditoria de segurança: funções SECURITY DEFINER que aceitavam tenant/usuário de outra loja.
-- Desfazer: _rollback_rls_funcoes_20260925.sql (definições originais).

-- Checagem única: libera servidor (service_role), chamada sem JWT (cron/SQL direto) e o dono da plataforma;
-- qualquer outro precisa ser membro da loja pedida.
create or replace function public._assert_tenant_access(p_tenant uuid)
returns void language plpgsql stable security definer set search_path = public as $$
begin
  if coalesce(auth.role(), '') in ('service_role', '') then return; end if;
  if auth.uid() is not null and (public.auth_is_member_of(p_tenant) or public.is_platform_owner(auth.uid())) then return; end if;
  raise exception 'Sem acesso a esta loja.' using errcode = '42501';
end $$;

create or replace function public._assert_self_user(p_user uuid)
returns void language plpgsql stable security definer set search_path = public as $$
begin
  if coalesce(auth.role(), '') in ('service_role', '') then return; end if;
  if auth.uid() is not null and (p_user = auth.uid() or public.is_platform_owner(auth.uid())) then return; end if;
  raise exception 'Sem acesso a este usuário.' using errcode = '42501';
end $$;
revoke all on function public._assert_tenant_access(uuid), public._assert_self_user(uuid) from public, anon;
grant execute on function public._assert_tenant_access(uuid), public._assert_self_user(uuid) to authenticated, service_role;


-- _fn_freelancer_pode: 'auth.uid() is null' liberava o anônimo.
create or replace function public._fn_freelancer_pode(p_tenant uuid) returns boolean language sql stable security definer set search_path to 'public' as $$
  select coalesce(auth.role(), '') in ('service_role', '') or (auth.uid() is not null and public.auth_is_member_of(p_tenant))
$$;

-- fn_card_providers(uuid)
...
AS $function$
  SELECT public._assert_tenant_access(p_tenant);
...
-- fn_check_stock_alert_for_items(uuid,jsonb)
...
-- fn_get_audit_log_v3(uuid,integer,timestamp with time zone,timestamp with time zone,text,text)
...
-- fn_get_ingredient_consumption_timeline(uuid,uuid,integer)
...
-- fn_get_ingredients_consumo(uuid)
...
-- fn_get_inventory_sessions_range(uuid,date,date)
...
-- fn_get_item_ingredients(uuid,uuid)
...
-- fn_get_opcoes_sem_estoque(uuid)
...
-- fn_get_orders_for_consumo(uuid,timestamp with time zone,timestamp with time zone)
...
-- fn_get_stock_critical_alerts(uuid)
...
-- fn_get_stock_movements(uuid,integer)
...
-- fn_get_stock_movements(uuid,integer,timestamp with time zone,timestamp with time zone,uuid)
...
-- fn_get_stock_movements_filtered(uuid,timestamp with time zone,timestamp with time zone,text[],uuid)
...
AS $function$
BEGIN
  PERFORM public._assert_self_user(p_user_id);
  UPDATE users
...
-- fn_next_tenant_order_number(uuid)
...
CREATE OR REPLACE FUNCTION public.fn_peek_senha(p_tenant_id text)
 ...
 SECURITY DEFINER
AS $function$
  SELECT public._assert_tenant_access(p_tenant_id::uuid); SELECT COALESCE((SELECT counter FROM senha_counter WHERE tenant_id = p_tenant_id), 200); $function$;

-- get_production_price_history(uuid,uuid)
...
AS $function$
  SELECT public._assert_self_user(p_user_id);
  SELECT ut.tenant_id, ut.role::text FROM public.user_tenants ut WHERE ut.user_id = p_user_id;
$function$;

-- Só o servidor chama estas.
revoke all on function public.fn_freelancer_do_extrato(uuid,text,date[],text) from public, anon, authenticated;
grant execute on function public.fn_freelancer_do_extrato(uuid,text,date[],text) to service_role;
revoke all on function public.fn_mp_sync_all() from public, anon, authenticated;
grant execute on function public.fn_mp_sync_all() to service_role;
revoke all on function public.fn_periodo_anterior_preview(uuid,date) from public, anon, authenticated;
grant execute on function public.fn_periodo_anterior_preview(uuid,date) to service_role;
revoke all on function public.fn_card_providers(uuid) from public, anon;
grant execute on function public.fn_card_providers(uuid) to authenticated, service_role;

-- Sessões de caixa abertas (valor de abertura) visíveis sem login.
drop policy if exists anon_select_sessions_open on public.sessions;
