-- Sugestão de vínculo: dizer se a nota vai entrar como compra (CMV) ou como despesa (2026-09-20)
--
-- A tela de confirmar vínculos anunciava "Importa a nota como despesa" só porque o modelo é 10
-- (NFS-e). Mas desde 2026-09-18, se o item daquela nota está classificado como CMV na
-- Classificação de Itens, o fiscal-inbound lança a nota como COMPRA — as notas de serviço da
-- Josiane (mão de obra que vira CMV na loja), por exemplo. A tela prometia despesa e entregava
-- compra. Agora a sugestão carrega a classe do item (match_detail.classe) e a tela diz a verdade.
-- Temp table renomeada para _cand2: 'create temp table if not exists' manteria a estrutura antiga
-- nas conexões já abertas do pooler.
...
begin
  ...
  loop
    ...
    select count(*) into v_n from _cand2 x
