-- fn_production_crud — 2026-08-06
--
-- Dois problemas corrigidos aqui:
--
-- 1) create_recipe/update_recipe IGNORAVAM `category`, `min_stock` e
--    `output_quantity`. O modal de ficha de producao mandava os tres, a tabela
--    tem as tres colunas, e a RPC gravava so name/unit/instructions/is_active.
--    Resultado: categoria e estoque minimo digitados pelo usuario eram
--    descartados em silencio (confirmado: as 2 fichas do banco com category
--    null e min_stock 0 apesar de terem sido cadastradas pela tela).
--    list_recipes/get_recipe tambem nao devolviam esses campos.
--
-- 2) `output_quantity` passa a ser gravavel. Ela define a ESCALA da receita:
--    os insumos da ficha rendem `output_quantity` unidades de saida, nao 1.
--    Sem isso, uma ficha com saida em `g` obrigava o usuario a lancar a receita
--    para 1 grama de produto — sem sentido pratico. Default continua 1, entao
--    fichas existentes mantem o comportamento antigo.
--
-- Aditivo e retrocompativel: os campos novos usam COALESCE contra o valor atual,
-- entao um payload sem eles nao altera nada.

CREATE OR REPLACE FUNCTION fn_production_crud(
  p_action   TEXT,
  p_user_id  UUID,
  p_tenant_id UUID,
  -- DEFAULT preservado da assinatura original: remove-lo faz o CREATE OR REPLACE
  -- falhar com 42P13 "cannot remove parameter defaults from existing function".
...
BEGIN
  ...
  ELSIF p_action = 'update_recipe' THEN
    ...
    SET
      ...
      -- 'category' aceita limpar (string vazia -> NULL) quando a chave vier no payload
