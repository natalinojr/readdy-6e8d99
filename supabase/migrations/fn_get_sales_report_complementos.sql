-- =============================================================================
-- MIGRATION: fn_get_sales_report — valor com complementos + Top Complementos
-- Data: 2026-07-13
-- Descricao:
--   1) TOP ITENS (top_items): a receita usava SUM(item_price * quantity). Em
--      pedidos de DELIVERY o item_price NAO inclui os complementos (ficam so em
--      order_item_options.additional_price), entao itens cujo valor vem dos
--      complementos (ex.: "Burritos Duo Mex") apareciam com R$ 0,00. Nos demais
--      canais (table/cashier/waiter/self_service) o item_price JA inclui os
--      complementos (o front soma no precoTotal antes de gravar). Por isso o
--      ajuste e' ORIGIN-AWARE: soma os complementos SOMENTE para origin_type
--      'delivery' — evita dupla contagem nos outros canais. avg_price passa a ser
--      receita/quantidade (coerente com o valor corrigido).
--   2) TOP COMPLEMENTOS (top_options): NOVO agregado — ranking dos complementos
--      escolhidos, com quantidade e receita (additional_price * quantity),
--      ordenado por receita. Alimenta a nova tabela "Top Complementos" no
--      relatorio (Visao Geral) e no financeiro (Visao Geral por sessao).
--   Ambos os overloads (com e sem p_session_id) sao atualizados.
-- =============================================================================
...
BEGIN
  ...
  -- Top itens — receita ORIGIN-AWARE: soma complementos so no delivery
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json)
  INTO v_top_items
  FROM (
    SELECT
      ...
      CASE WHEN SUM(oi.quantity) > 0
        THEN SUM((oi.item_price + CASE WHEN o.origin_type::text = 'delivery' THEN COALESCE(oo.opt_sum, 0) ELSE 0 END) * oi.quantity) / SUM(oi.quantity)
        ELSE 0 END AS avg_price
    FROM order_items oi
    JOIN orders o ON o.id = oi.order_id
    LEFT JOIN (
      SELECT order_item_id, SUM(additional_price) AS opt_sum
      FROM order_item_options
      WHERE tenant_id = p_tenant_id
      GROUP BY order_item_id
    ) oo ON oo.order_item_id = oi.id
  ...
  -- Top complementos — ranking dos complementos escolhidos (qtd + receita)
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json)
  INTO v_top_options
  FROM (
    SELECT
      TRIM(oio.option_name) AS option_name,
      SUM(oi.quantity) AS total_qty,
      SUM(oio.additional_price * oi.quantity) AS total_revenue
