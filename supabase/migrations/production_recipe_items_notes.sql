-- production_recipe_items.notes — 2026-08-11
--
-- Pedido do usuario: poder anotar uma observacao por insumo dentro da ficha
-- de producao (ex: "usar sempre fresco", "cortar em cubos pequenos",
-- "substituir por X se nao tiver"). Nao existia campo nenhum pra isso —
-- confirmado no schema antes de escrever esta migracao.
--
-- fn_production_crud precisa ler/gravar o campo em list_recipes, get_recipe,
-- create_recipe e update_recipe — os 4 pontos que ja lidam com items.

ALTER TABLE production_recipe_items ADD COLUMN IF NOT EXISTS notes text;
...
BEGIN
  ...
  ELSIF p_action = 'create_recipe' THEN
    ...
    LOOP
      INSERT INTO production_recipe_items (recipe_id, ingredient_id, ingredient_name, quantity, unit, unit_cost, notes)
      VALUES (
        v_recipe_id,
