...
--
-- Caso real: em 14/09 a Stone informou 52 vendas de débito (R$ 1.439,74) e o Inter creditou
-- R$ 1.427,87; a venda que faltou (R$ 11,87) entrou em 16/09 num crédito separado. Pela regra do
-- mesmo dia, 14/09 fica "faltou" e 16/09 "sobrou" para sempre.
--
-- Segunda passada em fn_match_card_deposits: para a mesma pilha (débito/antecipado), um dia D que
-- não fechou por FALTA e um dia E entre D+1 e D+5 (até 3 dias úteis) que não fechou por SOBRA, com
-- o arquivo da Stone de E já importado: se (banco D + banco E) = (Stone D + Stone E) dentro da
-- tolerância, concilia os dois juntos (match_group 'stone:D+E:normal|antecipado').
-- fn_stone_repasses ganha a situação 'atrasado' e a coluna par_dia (o outro dia do par).
...
begin
  ...
  if f.card_provider = 'stone' and f.card_deposit_account_id is not null and f.card_deposit_match is not null then
    -- 1ª passada: repasse do dia × créditos do mesmo dia
    for g in
      with st as (
    ...
    -- 2ª passada: parte do repasse de D creditada pelo banco num dia seguinte E (até D+5)
    for g in
      with st as (
        select transaction_date as d,
               coalesce((stone_installment_info->>'advance_fee')::numeric, 0) > 0 as adv,
          ...
          from fin_bank_statement_imports
         where tenant_id = p_tenant and source = 'stone' and raw->>'kind' = 'installment'
           and transaction_type = 'credit' and status = 'pending'
           and transaction_date between p_from and p_to + 5
         group by 1, 2
      ),
      dias_stone as (
        -- dias com arquivo da Stone (qualquer pilha, conciliada ou não)
      ...
      pares as (
        select a.d as d1, ib.d as d2, a.adv,
         ...
         where a.d <= p_to
           ...
           -- D faltou e E sobrou (cada um sozinho não fecha)
    ...
    loop
      -- um mesmo dia E não pode completar dois repasses: confere se tudo ainda está pendente
  ...
  return jsonb_build_object('groups', v_groups, 'inter_rows', v_dep, 'stone_rows', v_det, 'transfers', v_transfers,
                            'delayed_pairs', v_atrasados, 'card_provider', f.card_provider);
end;
$function$;

-- Quadro dos repasses com a situação 'atrasado' (o tipo de retorno muda: drop + create)
drop function if exists public.fn_stone_repasses(uuid, date, date);

create function public.fn_stone_repasses(p_tenant uuid, p_from date, p_to date)
returns table (
  ...
  dia_liquido_stone numeric, dia_depositado numeric, situacao text, par_dia date
)
language sql
stable
set search_path to 'public'
as $function$
  ...
  -- mesmo critério da 2ª passada de fn_match_card_deposits: D faltou, E (até D+5, com arquivo da Stone) sobrou, juntos fecham
  pares0 as (
  ...
  pares as (
    -- um dia E completa no máximo um dia D
