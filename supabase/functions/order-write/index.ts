...
Deno.serve({ verify_jwt: false }, async (req: Request) => {
  ...
  try {
    ...
    if (action === "create_order") {
      ...
      if (destination_phone && destination_name) { try { const { data: custData } = await admin.rpc("upsert_customer", { p_tenant_id: tenantId, p_name: destination_name, p_phone: destination_phone }); if (custData) { customerId = custData as string; } } catch { /* non-blocking */ } }
