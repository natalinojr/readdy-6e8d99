...
async function buildOptionDeductions(
  admin: ReturnType<typeof createClient>,
  tenantId: string,
  options: OrderItemOption[],
  baseQty: number,
): Promise<Array<{ ingredient_id: string; quantity: number; unit: string }>> {
  ...
  // Busca opções com ingredient_id direto
  const { data: optRows } = await admin
    .from("options")
    .select("id, ingredient_id, production_recipe_id, consumption_quantity, ingredients!inner(unit)")
    .in("id", validOptionIds)
    .eq("tenant_id", tenantId)
    .not("ingredient_id", "is", null);

  for (const row of (optRows ?? []) as Array<{
    id: string; ingredient_id: string | null; production_recipe_id: string | null;
    consumption_quantity: number | null; ingredients: { unit: string } | null;
  }>) {
    let ingredientId = row.ingredient_id;
    let consumptionQty = Number(row.consumption_quantity ?? 1);
    let stockUnit = row.ingredients?.unit ?? "unit";

    // Se é produção, busca o output_ingredient_id da receita
    if (!ingredientId && row.production_recipe_id) {
      const { data: recipeRow } = await admin
        .from("production_recipes")
        .select("output_ingredient_id, output_quantity, unit")
        .eq("id", row.production_recipe_id)
        .eq("tenant_id", tenantId)
        .maybeSingle();
      if (recipeRow?.output_ingredient_id) {
        ingredientId = recipeRow.output_ingredient_id;
        consumptionQty = Number(row.consumption_quantity ?? recipeRow.output_quantity ?? 1);
        stockUnit = recipeRow.unit ?? "unit";
      }
    }

    if (!ingredientId) continue;
    const totalQty = consumptionQty * baseQty;
    deductions.push({ ingredient_id: ingredientId, quantity: totalQty, unit: stockUnit });
  }
...
async function buildDeductions(
  ...
  const deductions: Array<{ ingredient_id: string; quantity: number; unit: string }> = [];
  if (itemId) {
    const { data: ingredients } = await admin.from("item_ingredients")
      .select("ingredient_id, quantity, unit, ingredients!inner(unit)")
      .eq("item_id", itemId)
      .eq("tenant_id", tenantId);
    for (const ing of (ingredients ?? []) as IngRow[]) {
      const fichaQty = Number(ing.quantity ?? 0) * baseQty;
      const fichaUnit = ing.unit ?? "unit";
      const stockUnit = ing.ingredients?.unit ?? "unit";
      deductions.push({ ingredient_id: ing.ingredient_id, quantity: convertUnitQty(fichaQty, fichaUnit, stockUnit), unit: stockUnit });
    }
  } else if (comboId) {
    const { data: comboIngredients } = await admin.from("combo_ingredients")
      .select("ingredient_id, quantity, unit, ingredients!inner(unit)")
      .eq("combo_id", comboId)
      .eq("tenant_id", tenantId);
    if (comboIngredients && comboIngredients.length > 0) {
      for (const ing of comboIngredients as IngRow[]) {
        const fichaQty = Number(ing.quantity ?? 0) * baseQty;
        const fichaUnit = ing.unit ?? "unit";
        const stockUnit = ing.ingredients?.unit ?? "unit";
        deductions.push({ ingredient_id: ing.ingredient_id, quantity: convertUnitQty(fichaQty, fichaUnit, stockUnit), unit: stockUnit });
  ...
  // Adiciona deduções das opções selecionadas
...
async function deductStockForOrderItem(admin: ReturnType<typeof createClient>, tenantId: string, orderId: string, orderItemId: string, operatorId: string): Promise<void> {
  const { data: orderItem } = await admin.from("order_items").select("item_id, combo_id, quantity, options").eq("id", orderItemId).maybeSingle();
  if (!orderItem) return;
  const qty = orderItem.quantity ?? 1;
  const options = (orderItem.options ?? []) as OrderItemOption[];
  ...
  for (const d of deductions) {
    ...
    if (existingMoves && existingMoves.length > 0) { log("INFO", "deductStockForOrderItem", "Baixa ja existente para este ingrediente neste pedido — ignorando duplicata", { order_id: orderId, ingredient_id: d.ingredient_id, order_item_id: orderItemId }); continue; }
  ...
  log("INFO", "deductStockForOrderItem", "Estoque baixado com sucesso", { order_item_id: orderItemId, order_id: orderId, moves_count: allMoves.length, ingredients_updated: deltaMap.size });
}
...
Deno.serve({ verify_jwt: false }, async (req: Request) => {
  ...
  if (!supabaseUrl) { log("ERROR", "init", "SUPABASE_URL nao configurado"); return new Response(JSON.stringify({ error: "Server misconfiguration" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
  ...
  void db;
  if (!serviceRoleKey || serviceRoleKey.length < 40) { log("ERROR", "init", "SUPABASE_SERVICE_ROLE_KEY nao configurado"); return new Response(JSON.stringify({ error: "Server misconfiguration" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
  const admin = createClient(supabaseUrl, serviceRoleKey, { auth: { autoRefreshToken: false, persistSession: false } });
  let jwtUserId: string;
  try {
    const token = authHeader.replace(/^Bearer\s+/i, "").trim();
    if (!token) { log("WARN", "auth", "Requisicao sem token JWT"); return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
    const { data: { user }, error: userError } = await admin.auth.getUser(token);
    if (userError || !user) { log("WARN", "auth", "Token JWT invalido ou expirado", { error: userError?.message }); return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
    jwtUserId = user.id;
  ...
  try {
    ...
    const isKDSAction = action === "update_order_item_status" || action === "update_unit_status";
    if (action === "create_order" && !requestedTenantId) { log("ERROR", "create_order", "BLOQUEADO: tenant_id ausente", { userId: jwtUserId }); return new Response(JSON.stringify({ error: "tenant_id is required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
    let tenantId: string;
    if (isKDSAction && requestedTenantId) {
      ...
      if (tenantCheckErr) { log("ERROR", action, "Falha ao validar tenant para KDS action", { userId: jwtUserId, error: tenantCheckErr.message }); return new Response(JSON.stringify({ error: "Tenant validation failed" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (!tenantCheck) {
        const { data: tenantExists } = await admin.from("tenants").select("id").eq("id", requestedTenantId).maybeSingle();
        if (!tenantExists) { log("ERROR", action, "BLOQUEADO: tenant_id invalido para KDS action", { userId: jwtUserId, requestedTenantId }); return new Response(JSON.stringify({ error: "Invalid tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      if (tenantErr) { log("ERROR", action, "Falha ao buscar tenant do usuario — BLOQUEADO", { userId: jwtUserId, error: tenantErr.message }); return new Response(JSON.stringify({ error: `Tenant lookup failed: ${tenantErr.message}` }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (!tenantRows || tenantRows.length === 0) { log("ERROR", action, "Usuario sem tenant — BLOQUEADO", { userId: jwtUserId }); return new Response(JSON.stringify({ error: "User does not belong to any tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (requestedTenantId) {
        const match = tenantRows.find((r: { tenant_id: string }) => r.tenant_id === requestedTenantId);
        if (!match) { log("ERROR", action, "BLOQUEADO: tenant_id nao pertence ao usuario", { userId: jwtUserId, requestedTenantId }); return new Response(JSON.stringify({ error: "User does not belong to the requested tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
        tenantId = match.tenant_id;
      } else if (tenantRows.length === 1) { tenantId = tenantRows[0].tenant_id; }
      else { log("ERROR", action, "Multiplos tenants — tenant_id obrigatorio", { userId: jwtUserId }); return new Response(JSON.stringify({ error: "Multiple tenants found — tenant_id required" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
    }
    ...
    if (action === "retry_order_items") {
      ...
      if (!orderCheck) { log("ERROR", "retry_order_items", "order_id nao encontrado no banco", { order_id, tenantId }); return new Response(JSON.stringify({ error: "Pedido nao encontrado" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      if (!tenantMembership) { log("ERROR", "retry_order_items", "BLOQUEADO: pedido pertence a tenant sem acesso do usuario", { order_id, orderTenantId, jwtUserId }); return new Response(JSON.stringify({ error: "Acesso negado a este pedido" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      log("INFO", "retry_order_items", "Iniciando retry de itens", { order_id, order_number: orderCheck.number, tenant_id: effectiveTenantId, expected: items.length, already_inserted: existingCount ?? 0 });
      if ((existingCount ?? 0) >= items.length) { log("INFO", "retry_order_items", "Itens ja inseridos — retry desnecessario", { order_id, existing: existingCount, expected: items.length }); return new Response(JSON.stringify({ ok: true, inserted: existingCount ?? 0, skipped: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      catch (itemsErr) { log("ERROR", "retry_order_items", "fn_create_order_items_bypass falhou no retry", { order_id, error: String(itemsErr) }); return new Response(JSON.stringify({ ok: false, inserted: 0, error: String(itemsErr) }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      log("INFO", "retry_order_items", "Retry concluido", { order_id, order_number: orderCheck.number, tenant_id: effectiveTenantId, before: existingCount ?? 0, after: afterCount ?? 0, newly_inserted: newlyInserted });
      try { const { data: allItems } = await admin.from("order_items").select("status, skip_kds").eq("order_id", order_id).neq("status", "cancelled"); if (allItems && allItems.length > 0) { const correctStatus = deriveOrderStatus(allItems as { status: string; skip_kds: boolean }[]); await admin.from("orders").update({ status: correctStatus, updated_at: new Date().toISOString() }).eq("id", order_id); } } catch (statusErr) { log("WARN", "retry_order_items", "Recalculo de status falhou (non-blocking)", { error: String(statusErr) }); }
    ...
    if (action === "open_cash_register") {
      ...
      if (!sessionCheck) { log("ERROR", "open_cash_register", "BLOQUEADO: session_id nao pertence ao tenant", { session_id, tenantId }); return new Response(JSON.stringify({ error: "session_id invalido para este tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      await ensureUserExists(effectiveUserId);
    ...
    if (action === "close_cash_register") {
      ...
      if (!crCheck) { log("ERROR", "close_cash_register", "BLOQUEADO: cash_register nao pertence ao tenant", { cash_register_id, tenantId }); return new Response(JSON.stringify({ error: "cash_register_id invalido para este tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      } catch (calcErr) { log("WARN", "close_cash_register", "Erro ao calcular closing_value_expected", { error: String(calcErr) }); }
    ...
    if (action === "add_cash_movement") {
      ...
      if (cash_register_id) {
        ...
        if (!crCheck) { log("ERROR", "add_cash_movement", "BLOQUEADO: cash_register nao pertence ao tenant", { cash_register_id, tenantId }); return new Response(JSON.stringify({ error: "cash_register_id invalido para este tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      }
      ...
      if (error) throw error;
      try { await admin.from("fin_cash_flow").insert({ tenant_id: tenantId, type: isOutflow ? "expense" : "income", amount, description: reason || (isOutflow ? "Sangria" : "Suprimento"), category: isOutflow ? "Sangria" : "Suprimento", origin: isOutflow ? "auto_sangria" : "auto_suprimento", reference_id: data?.id, date: todayBrasiliaStr() }); } catch (e) { log("WARN", "add_cash_movement", "Erro ao registrar no fin_cash_flow", { error: String(e) }); }
      return new Response(JSON.stringify({ data }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    if (action === "create_order") {
      // ── CORREÇÃO: extrair table_session_id do body para vincular pedido à sessão de mesa ──
      ...
      log("INFO", "create_order", "Iniciando criacao de pedido", { tenant_id: tenantId, session_id: session_id ?? "AUSENTE", origin: origin ?? "AUSENTE", destination: destination ?? "AUSENTE", item_count: Array.isArray(items) ? items.length : 0, subtotal: subtotal ?? 0, total_amount: total_amount ?? 0, user_id: effectiveUserId, is_training: is_training ?? false, delivery_platform: deliveryPlatform, table_session_id: table_session_id ?? null });
      if (!session_id) { log("ERROR", "create_order", "BLOQUEADO: session_id ausente", { tenant_id: tenantId, origin, user_id: effectiveUserId }); return new Response(JSON.stringify({ error: "session_id is required — pedido bloqueado sem sessao ativa" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (!Array.isArray(items) || items.length === 0) { log("ERROR", "create_order", "BLOQUEADO: items vazio ou ausente", { tenant_id: tenantId, session_id, origin, user_id: effectiveUserId }); return new Response(JSON.stringify({ error: "items array is required and must not be empty" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (subtotal != null && total_amount != null) {
        ...
        if (diff > 0.01) { log("ERROR", "create_order", "BLOQUEADO: inconsistencia financeira no pedido", { subtotal, discount_amount, service_fee_amount, total_amount, expected_total: expectedTotal, diff, tenant_id: tenantId, session_id }); return new Response(JSON.stringify({ error: `Inconsistencia financeira: subtotal(${subtotal}) - desconto(${discount_amount ?? 0}) + taxa(${service_fee_amount ?? 0}) = ${expectedTotal.toFixed(2)}, mas total_amount enviado = ${total_amount}. Diferenca: ${diff.toFixed(2)}` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      }
      const { data: sessionRow, error: sessionErr } = await admin.from("sessions").select("id, tenant_id, status").eq("id", session_id).maybeSingle();
      if (sessionErr) { log("WARN", "create_order", "Erro ao validar session_id — prosseguindo com cautela", { session_id, tenant_id: tenantId, error: sessionErr.message }); }
      else if (!sessionRow) { log("ERROR", "create_order", "BLOQUEADO: session_id nao encontrado no banco", { session_id, tenant_id: tenantId, origin }); return new Response(JSON.stringify({ error: "session_id invalido — sessao nao encontrada" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      else if (sessionRow.tenant_id !== tenantId) { log("ERROR", "create_order", "BLOQUEADO: session_id pertence a outro tenant", { session_id, session_tenant: sessionRow.tenant_id, request_tenant: tenantId, origin }); return new Response(JSON.stringify({ error: "session_id nao pertence ao tenant informado" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      await ensureUserExists(effectiveUserId);
      let customerId: string | null = null;
      if (destination_phone && destination_name) {
        try {
          const { data: custData, error: custErr } = await admin.rpc("upsert_customer", { p_tenant_id: tenantId, p_name: destination_name, p_phone: destination_phone });
          if (custErr) { log("WARN", "create_order", "upsert_customer retornou erro (non-blocking)", { error: custErr.message, tenant_id: tenantId, phone: destination_phone }); }
          else if (custData) { customerId = custData as string; }
        } catch (e) { log("WARN", "create_order", "upsert_customer excecao (non-blocking)", { error: String(e), tenant_id: tenantId, phone: destination_phone }); }
      }
      const numData = await withRetry(async () => { const { data, error } = await admin.rpc("fn_next_order_number", { p_session_id: session_id }); if (error) throw error; return data; }, 3, "create_order:fn_next_order_number", { session_id, tenant_id: tenantId });
      ...
      // Resolve table_session_id: only use valid UUID
      ...
      if (!orderId) { log("ERROR", "create_order", "fn_create_order_bypass retornou sem ID", { session_id, tenant_id: tenantId, origin: mappedOrigin, orderNumber }); return new Response(JSON.stringify({ error: "Order created but no ID returned" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      if (deliveryPlatform) { try { await admin.from("orders").update({ delivery_platform: deliveryPlatform }).eq("id", orderId); log("INFO", "create_order", "delivery_platform salvo", { order_id: orderId, delivery_platform: deliveryPlatform }); } catch (platErr) { log("WARN", "create_order", "Falha ao salvar delivery_platform (non-blocking)", { error: String(platErr) }); } }
      log("INFO", "create_order", "Pedido criado no banco — inserindo itens", { order_id: orderId, order_number: orderNumber, tenant_id: tenantId, session_id, origin: mappedOrigin, item_count: items.length, table_session_id: resolvedTableSessionId });
      const resolvedItems: Array<Record<string, unknown>> = [];
      for (const item of (items ?? [])) {
        let resolvedItemId: string | null = null;
        if (isValidUuid(item.item_id)) resolvedItemId = item.item_id;
      ...
      if (resolvedItems.length > 0) {
        ...
        try {
          ...
          log("INFO", "create_order", "fn_create_order_items_bypass concluido sem excecao", { order_id: orderId, order_number: orderNumber, tenant_id: tenantId, item_count: resolvedItems.length });
        } catch (itemsErr) {
          const errMsg = itemsErr instanceof Error ? itemsErr.message : String(itemsErr);
          log("ERROR", "create_order", "FALHA CRITICA: fn_create_order_items_bypass apos 3 tentativas", { order_id: orderId, order_number: orderNumber, tenant_id: tenantId, session_id, origin: mappedOrigin, item_count: resolvedItems.length, error: errMsg });
          try { const { count: countAfterErr } = await admin.from("order_items").select("id", { count: "exact", head: true }).eq("order_id", orderId).eq("tenant_id", tenantId); if ((countAfterErr ?? 0) > 0) { log("WARN", "create_order", "Itens encontrados apesar da excecao", { order_id: orderId, order_number: orderNumber, inserted: countAfterErr }); itemsInsertedOk = true; } } catch (cntErr) { log("WARN", "create_order", "Erro ao verificar count pos-falha", { error: String(cntErr) }); }
        }
        if (!itemsInsertedOk) {
          let finalCount = 0;
          try { const { count } = await admin.from("order_items").select("id", { count: "exact", head: true }).eq("order_id", orderId).eq("tenant_id", tenantId); finalCount = count ?? 0; } catch { /* ignora */ }
          if (finalCount === 0) {
            log("ERROR", "create_order", "FALHA TOTAL confirmada: 0 itens — cancelando pedido", { order_id: orderId, order_number: orderNumber });
            try { await admin.from("orders").update({ status: "cancelled", cancel_reason: "Auto-cancelado: falha critica ao inserir itens", cancelled_at: new Date().toISOString(), cancelled_by: effectiveUserId }).eq("id", orderId); } catch (cancelErr) { log("WARN", "create_order", "Falha ao auto-cancelar pedido", { error: String(cancelErr) }); }
          ...
          } else { log("WARN", "create_order", "Itens encontrados na verificacao final", { order_id: orderId, order_number: orderNumber, count: finalCount }); itemsInsertedOk = true; }
        }
        if (itemsInsertedOk) {
          try { const { count: insertedCount } = await admin.from("order_items").select("id", { count: "exact", head: true }).eq("order_id", orderId).eq("tenant_id", tenantId); const actualCount = insertedCount ?? 0; if (actualCount < resolvedItems.length) { log("WARN", "create_order", "INSERCAO PARCIAL: alguns itens foram perdidos", { order_id: orderId, order_number: orderNumber, expected: resolvedItems.length, inserted: actualCount }); } else { log("INFO", "create_order", "Itens inseridos com sucesso", { order_id: orderId, order_number: orderNumber, item_count: actualCount }); } } catch { /* non-blocking */ }
          try {
            ...
            } catch (unitFixErr) { log("WARN", "create_order", "skip_kds unit creation failed (non-blocking)", { error: String(unitFixErr) }); }
            log("INFO", "create_order", "skip_kds items marcados como prontos individualmente", { order_id: orderId, order_number: orderNumber, is_delivery: isDeliveryOrigin, all_skip_kds: allSkipKds });
          } catch (statusFixErr) { log("WARN", "create_order", "Garantia de status falhou (non-blocking)", { error: String(statusFixErr) }); }
        }
        for (const item of resolvedItems) {
          if (item.skip_kds) {
            ...
            } catch (stockErr) { log("WARN", "create_order", "stock deduct (skip_kds) falhou (non-blocking)", { error: String(stockErr) }); }
          }
        }
      }
      log("INFO", "create_order", "Pedido finalizado com sucesso", { order_id: orderId, order_number: orderNumber, tenant_id: tenantId, session_id, origin: mappedOrigin, destination: mappedDest, item_count: resolvedItems.length, total_amount: total_amount ?? 0, delivery_platform: deliveryPlatform, table_session_id: resolvedTableSessionId });
    ...
    if (action === "apply_discount") {
      ...
      if (requires_approval && !approved_by) { log("ERROR", "apply_discount", "BLOQUEADO: requires_approval=true mas approved_by ausente", { order_id, tenant_id: tenantId }); return new Response(JSON.stringify({ error: "approved_by e obrigatorio quando requires_approval=true" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      const { data: orderCheck } = await admin.from("orders").select("id, tenant_id").eq("id", order_id).maybeSingle();
      if (!orderCheck) { log("ERROR", "apply_discount", "BLOQUEADO: order_id nao encontrado", { order_id }); return new Response(JSON.stringify({ error: "Pedido nao encontrado" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      if (!discountTenantMembership) { log("ERROR", "apply_discount", "BLOQUEADO: sem acesso ao tenant do pedido", { order_id, orderTenantCheck, jwtUserId }); return new Response(JSON.stringify({ error: "Acesso negado a este pedido" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
    ...
    if (action === "update_order_item_status") {
      ...
      if (dbStatus === "ready" || dbStatus === "delivered") { deductStockForOrderItem(admin, tenantId, order_id, order_item_id, effectiveUserId).catch((e) => log("WARN", "update_order_item_status", "deductStockForOrderItem falhou (non-blocking)", { error: String(e) })); }
      try {
      ...
      } catch (unitSyncErr) { log("WARN", "update_order_item_status", "unit sync error (non-blocking)", { error: String(unitSyncErr) }); }
    ...
    if (action === "update_unit_status") {
      ...
      const dbStatus = STATUS_TO_DB[status] ?? status;
      const now = new Date().toISOString();
      const { data: unitRow, error: unitFindErr } = await admin.from("order_item_units").select("id, status").eq("order_item_id", order_item_id).eq("unit_number", unit_number).maybeSingle();
      if (unitFindErr) log("WARN", "update_unit_status", "Erro ao buscar unit", { error: unitFindErr.message });
      const unitUpdate: Record<string, unknown> = { status: dbStatus };
      if (dbStatus === "preparing") unitUpdate.started_preparing_at = now;
      if (dbStatus === "ready") unitUpdate.ready_at = now;
      if (dbStatus === "delivered") { unitUpdate.delivered_at = now; unitUpdate.delivered_by_user_id = effectiveUserId; }
      if (unitRow?.id) {
        ...
        const newRank = STATUS_RANK[dbStatus] ?? 0;
        if (newRank > currentRank) { const { error: unitUpdateErr } = await admin.from("order_item_units").update(unitUpdate).eq("id", unitRow.id); if (unitUpdateErr) { log("ERROR", "update_unit_status", "Erro ao atualizar unit", { error: unitUpdateErr.message }); throw unitUpdateErr; } }
      } else {
        const { data: itemRow } = await admin.from("order_items").select("tenant_id, station_id").eq("id", order_item_id).maybeSingle();
        const { error: unitInsertErr } = await admin.from("order_item_units").insert({ order_item_id, tenant_id: itemRow?.tenant_id ?? tenantId, unit_number, station_id: itemRow?.station_id ?? null, operator_id: effectiveUserId, entered_kds_at: now, ...unitUpdate });
        if (unitInsertErr) log("WARN", "update_unit_status", "Erro ao inserir unit", { error: unitInsertErr.message });
      }
      ...
      if (allUnitsDelivered) { newItemStatus = "delivered"; deductStockForOrderItem(admin, tenantId, order_id, order_item_id, effectiveUserId).catch((e) => log("WARN", "update_unit_status", "deductStockForOrderItem (unit delivered) falhou (non-blocking)", { error: String(e) })); }
      else if (allUnitsReady && newItemStatus !== "ready" && newItemStatus !== "delivered") { newItemStatus = "ready"; deductStockForOrderItem(admin, tenantId, order_id, order_item_id, effectiveUserId).catch((e) => log("WARN", "update_unit_status", "deductStockForOrderItem (unit ready) falhou (non-blocking)", { error: String(e) })); }
    ...
    if (action === "cancel_order") {
      ...
      if (!orderCheck) { log("ERROR", "cancel_order", "BLOQUEADO: order_id nao pertence ao tenant", { order_id, tenantId }); return new Response(JSON.stringify({ error: "order_id invalido para este tenant" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      log("INFO", "cancel_order", "Cancelando pedido", { order_id, reason, tenant_id: tenantId });
      ...
      if (reason_type && refund_amount != null && refund_method) {
        try {
          ...
          if (restock_items) {
            try { const { data: orderItems } = await admin.from("order_items").select("id, item_id, combo_id, quantity").eq("order_id", order_id).eq("tenant_id", tenantId); if (orderItems && orderItems.length > 0) for (const oi of orderItems) await restockForOrderItem(admin, tenantId, order_id, oi.id, effectiveUserId); }
            catch (stockErr) { log("WARN", "cancel_order", "restock falhou (non-blocking)", { error: String(stockErr) }); }
          }
          try { await admin.from("audit_log").insert({ tenant_id: tenantId, user_id: effectiveUserId, action_type: "order_refunded", entity_type: "order", entity_id: order_id, details: { refund_id: refundRecord?.id ?? null, reason_type, refund_amount, refund_method, restock_items: restock_items ?? false, payment_id: payment?.id ?? null } }); } catch (auditErr) { log("WARN", "cancel_order", "audit_log order_refunded falhou (non-blocking)", { error: String(auditErr) }); }
        } catch (refundFlowErr) { log("WARN", "cancel_order", "refund flow error (non-blocking)", { error: String(refundFlowErr) }); }
      }
      return new Response(JSON.stringify({ ok: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "process_refund") {
      ...
      if (refund.payment_id) await admin.from("payments").update({ is_refunded: true }).eq("id", refund.payment_id).catch((e: unknown) => log("WARN", "process_refund", "marcar pagamento como estornado falhou", { error: String(e) }));
      try { await admin.from("fin_cash_flow").insert({ tenant_id: tenantId, type: "expense", amount: refund.refund_amount, description: `Estorno pedido #${refund.order_id.slice(0, 8)} (${refund.refund_method})`, category: "Estornos", origin: "manual", reference_id: refund_id, date: todayBrasiliaStr() }); } catch (flowErr) { log("WARN", "process_refund", "fin_cash_flow estorno falhou (non-blocking)", { error: String(flowErr) }); }
      try { await admin.from("audit_log").insert({ tenant_id: tenantId, user_id: effectiveUserId, action_type: "refund_processed", entity_type: "refund", entity_id: refund_id, details: { order_id: refund.order_id, refund_amount: refund.refund_amount, refund_method: refund.refund_method, approved_by: effectiveUserId } }); } catch (auditErr) { log("WARN", "process_refund", "audit_log refund_processed falhou (non-blocking)", { error: String(auditErr) }); }
      return new Response(JSON.stringify({ ok: true, status: "processed" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    if (action === "record_payment") {
      const { order_id, cash_register_id, payment_method_id, amount, change_amount, operator_name } = body;
      if (!payment_method_id) { log("WARN", "record_payment", "Skipped — missing payment_method_id", { order_id, tenant_id: tenantId }); return new Response(JSON.stringify({ data: null, skipped: true, reason: "missing_payment_method_id" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      let orderTotalAmount = 0;
      if (order_id) {
        const { data: orderCheck } = await admin.from("orders").select("id, tenant_id, status, total_amount").eq("id", order_id).maybeSingle();
        if (!orderCheck) { log("WARN", "record_payment", "order_id nao encontrado — registrando pagamento sem validacao de pedido", { order_id, tenantId }); }
        else {
          const orderTenantId = orderCheck.tenant_id as string;
          ...
          if (!paymentTenantMembership) { log("ERROR", "record_payment", "BLOQUEADO: sem acesso ao tenant do pedido", { order_id, orderTenantId, jwtUserId }); return new Response(JSON.stringify({ error: "Acesso negado a este pedido" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
          if (orderCheck.status === "cancelled") { log("WARN", "record_payment", "BLOQUEADO: tentativa de pagamento em pedido cancelado", { order_id, tenantId, status: orderCheck.status }); return new Response(JSON.stringify({ error: "Nao e possivel registrar pagamento em pedido cancelado", skipped: true }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      if (!effectiveCashRegisterId && order_id) {
        try { const { data: orderRow } = await admin.from("orders").select("session_id").eq("id", order_id).maybeSingle(); if (orderRow?.session_id) { const { data: crRow } = await admin.from("cash_registers").select("id").eq("session_id", orderRow.session_id).eq("status", "open").order("opened_at", { ascending: false }).limit(1).maybeSingle(); effectiveCashRegisterId = crRow?.id ?? null; } } catch (e) { log("WARN", "record_payment", "Falha ao buscar cash_register da sessao", { error: String(e) }); }
      }
      if (!effectiveCashRegisterId) log("WARN", "record_payment", "Sem cash_register_id — registrando apenas no financeiro", { order_id, tenant_id: paymentTenantId, payment_method_id, amount });
      const { data: paymentId, error: payErr } = await admin.rpc("fn_record_payment_bypass", {
        p_order_id: order_id,
        p_tenant_id: paymentTenantId,
        p_cash_register_id: effectiveCashRegisterId,
        p_payment_method_id: payment_method_id,
        p_amount: amount,
        p_change_amount: change_amount ?? 0,
        p_operator_name: operator_name ?? null,
      });
      if (payErr) { log("ERROR", "record_payment", "fn_record_payment_bypass error", { order_id, tenant_id: paymentTenantId, error: payErr.message }); throw payErr; }
      if (order_id) {
        try {
          ...
          if (orderTotalAmount > 0 && totalPaid >= orderTotalAmount) { const paidAt = new Date().toISOString(); await admin.from("orders").update({ is_paid: true, paid_at: paidAt, updated_at: paidAt }).eq("id", order_id); log("INFO", "record_payment", "orders.is_paid marcado como true", { order_id, total_paid: totalPaid, order_total: orderTotalAmount }); }
          else if (orderTotalAmount === 0) { const paidAt = new Date().toISOString(); await admin.from("orders").update({ is_paid: true, paid_at: paidAt, updated_at: paidAt }).eq("id", order_id); }
        } catch (paidUpdateErr) { log("WARN", "record_payment", "Atualizacao de is_paid falhou (non-blocking)", { order_id, error: String(paidUpdateErr) }); }
      }
      try {
        ...
        if (orderData?.customer_id) {
          ...
          if (pointsEarned > 0) await admin.from("loyalty_transactions").insert({ tenant_id: paymentTenantId, customer_id: customerId, transaction_type: "earned", points: pointsEarned, balance_after: newPoints, order_id, notes: `Compra #${orderData.number ?? order_id.slice(0, 8)}`, created_by: effectiveUserId }).catch((e: unknown) => log("WARN", "record_payment", "loyalty_transactions insert falhou (non-blocking)", { error: String(e) }));
        }
      } catch (custErr) { log("WARN", "record_payment", "customer/loyalty update falhou (non-blocking)", { error: String(custErr) }); }
      try {
        ...
        if (daysToReceive === 0) {
          ...
          if (paymentIdStr) {
            ...
            else { log("INFO", "record_payment", "fin_cash_flow ja existe para este pagamento — ignorando duplicata", { payment_id: paymentIdStr, order_id, amount: saleAmount }); }
            const { data: routing, error: routingErr } = await admin.from("fin_income_routing").select("bank_account_id").eq("tenant_id", paymentTenantId).eq("source_type", "payment_method").eq("source_id", payment_method_id).eq("is_active", true).maybeSingle();
            if (routingErr) { log("WARN", "record_payment", "Erro ao buscar roteamento", { error: routingErr.message, payment_method_id }); }
            else if (routing?.bank_account_id) { const { error: creditErr } = await admin.rpc("fn_bank_credit", { p_bank_account_id: routing.bank_account_id, p_amount: saleAmount, p_description: paymentDesc, p_reference_type: "sale", p_reference_id: order_id, p_transaction_date: todayBR }); if (creditErr) { log("WARN", "record_payment", "fn_bank_credit falhou (non-blocking)", { error: creditErr.message, bank_account_id: routing.bank_account_id, amount: saleAmount, order_id }); } else { log("INFO", "record_payment", "Receita roteada para conta bancaria", { bank_account_id: routing.bank_account_id, payment_method_id, amount: saleAmount, order_id }); } }
            else { log("INFO", "record_payment", "Sem roteamento configurado — apenas fluxo de caixa", { payment_method_id, order_id }); }
            ...
            if (feePercent > 0) {
              ...
              if (feeAmount > 0) {
                ...
                if (!existingFee) { await admin.from("fin_cash_flow").insert({ tenant_id: paymentTenantId, type: "expense", amount: feeAmount, description: feeDesc, category: "Taxas de Cartao", origin: "auto_card_fee", reference_id: paymentIdStr, date: todayBR }); log("INFO", "record_payment", "Taxa da maquininha registrada automaticamente", { payment_id: paymentIdStr, fee_percent: feePercent, fee_amount: feeAmount, sale_amount: saleAmount, payment_method: pmData?.name }); }
                else { log("INFO", "record_payment", "Taxa da maquininha ja existia — ignorando duplicata", { payment_id: paymentIdStr }); }
              }
            }
          } else {
            log("WARN", "record_payment", "paymentId nulo — registrando fin_cash_flow sem reference_id", { order_id, tenant_id: paymentTenantId, amount });
            await admin.from("fin_cash_flow").insert({ tenant_id: paymentTenantId, type: "income", amount: orderTotalAmount > 0 ? orderTotalAmount : amount, description: `${paymentDesc} (sem ref)`, category: "Vendas", origin: "auto_sale", reference_id: null, date: todayBR });
          }
        } else {
          ...
          if (installErr) { log("WARN", "record_payment", "Erro ao inserir recebivel — usando fallback D+0", { error: installErr.message }); await admin.from("fin_cash_flow").insert({ tenant_id: paymentTenantId, type: "income", amount, description: `${paymentDesc} — fallback`, category: "Vendas", origin: "auto_sale", reference_id: String(paymentId ?? order_id), date: todayBR }); }
        }
      } catch (e) {
        log("WARN", "record_payment", "Erro ao processar prazo de recebimento — usando fallback D+0", { error: String(e) });
        try { await admin.from("fin_cash_flow").insert({ tenant_id: paymentTenantId, type: "income", amount, description: `Venda #${order_id.slice(0, 8)}`, category: "Vendas", origin: "auto_sale", reference_id: String(paymentId ?? order_id), date: todayBrasiliaStr() }); }
        catch (e2) { log("WARN", "record_payment", "Fallback fin_cash_flow tambem falhou", { error: String(e2) }); }
      }
    ...
    if (action === "update_order_item") {
      const { order_id, order_item_id, quantity, notes, observations, options } = body;
      ...
      if (observations && Array.isArray(observations)) {
        const normalizedObservations = (observations as Array<Record<string, unknown>>).filter((o) => o.text && String(o.text).trim() !== "").map((o) => ({ order_item_id, tenant_id: orderTenantId, text: String(o.text).trim(), is_checked: o.is_checked === true }));
        ...
        if (normalizedObservations.length > 0) {
          ...
          if (obsErr) { log("WARN", "update_order_item", "Erro ao inserir observacoes", { error: obsErr.message }); return new Response(JSON.stringify({ error: `Erro ao salvar observacao: ${obsErr.message}` }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
        }
        log("INFO", "update_order_item", "Observacoes salvas com sucesso", { order_item_id, count: normalizedObservations.length });
      }
      if (quantity != null && typeof quantity === "number" && quantity > 0) {
        ...
        log("INFO", "update_order_item", "Totais recalculados apos edicao de quantidade", { order_id, new_subtotal: newSubtotal, new_total: newTotal });
      }
      try { const { data: allItemsStatus } = await admin.from("order_items").select("status, skip_kds").eq("order_id", order_id).neq("status", "cancelled"); if (allItemsStatus && allItemsStatus.length > 0) { const newOrderStatus = deriveOrderStatus(allItemsStatus as { status: string; skip_kds: boolean }[]); await admin.from("orders").update({ status: newOrderStatus, updated_at: now }).eq("id", order_id); } } catch (statusErr) { log("WARN", "update_order_item", "Recalculo de status falhou (non-blocking)", { error: String(statusErr) }); }
      return new Response(JSON.stringify({ ok: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "upsert_division_customers") {
      ...
      if (rows.length > 0) {
        ...
        if (error) { log("ERROR", "upsert_division_customers", "Erro ao inserir clientes", { error: JSON.stringify(error) }); throw error; }
        return new Response(JSON.stringify({ data }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      return new Response(JSON.stringify({ data: [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    log("WARN", action ?? "unknown", "Action nao reconhecida", { action, tenant_id: tenantId });
    return new Response(JSON.stringify({ error: "Unknown action" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : typeof err === "object" ? JSON.stringify(err) : typeof err === "string" ? err : String(err);
