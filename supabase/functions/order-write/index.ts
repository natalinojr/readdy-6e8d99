...
// Depois que um pedido de balcão/delivery fica pago, pede à fiscal-write que emita a
// NFC-e. Pedidos de mesa NÃO passam aqui: a nota da mesa sai no fechamento da sessão
// (table-write). Só chama a função se a loja tem emissão ligada (1 select barato),
// e nunca bloqueia a resposta do PDV.
async function triggerFiscalEmit(admin: ReturnType<typeof createClient>, supabaseUrl: string, _serviceRoleKey: string, tenantId: string, orderId: string) {
  try {
    ...
    const p = fetch(`${supabaseUrl}/functions/v1/fiscal-write`, {
      ...
      body: JSON.stringify({ action: "emit", tenant_id: tenantId, source_type: "order", source_id: orderId, trigger: "record_payment" }),
...
Deno.serve({ verify_jwt: false }, async (req: Request) => {
  ...
  try {
    ...
    if (action === "record_payment") {
      ...
      // NFC-e automática (balcão/delivery). Pedido de mesa é ignorado aqui (nota sai no fechamento da sessão).
      if (becamePaid && order_id) { try { const { data: oFis } = await admin.from("orders").select("table_session_id, is_training").eq("id", order_id).maybeSingle(); if (oFis && !oFis.table_session_id && !oFis.is_training) await triggerFiscalEmit(admin, supabaseUrl, serviceRoleKey, paymentTenantId, order_id); } catch { /* non-blocking */ } }
