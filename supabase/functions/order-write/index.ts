...
// Depois que um pedido fica pago, pede à fiscal-write que emita a NFC-e. Ela pula os
// pedidos de mesa NUMERADA (a nota dessa sai no fechamento da sessão, via table-write);
// balcão, delivery e QR universal ("Mesa 0") saem por pedido. Só chama a função se a loja tem emissão ligada (1 select barato),
// e nunca bloqueia a resposta do PDV.
async function triggerFiscalEmit(admin: ReturnType<typeof createClient>, supabaseUrl: string, _serviceRoleKey: string, tenantId: string, orderId: string, group?: { id: string; size: number } | null) {
  try {
    ...
    }).then(async (r) => { const t = await r.text().catch(() => ""); log("INFO", "fiscal", "fiscal-write emit", { orderId, http: r.status, body: t.slice(0, 300) }); })
      .catch((e) => log("WARN", "fiscal", "fiscal-write falhou", { orderId, error: String(e) }));
...
// ── Libera o pedido do autoatendimento segurado até o pagamento (dinheiro no caixa) ──
// Sai do rascunho = vai pro KDS/gestor; itens sem preparo baixam o estoque agora. Os tickets
// de cozinha o PDV que recebeu enfileira (mesmo caminho do pedido do caixa). Idempotente.
async function releaseHeldSelfServiceOrder(admin: ReturnType<typeof createClient>, tenantId: string, orderId: string): Promise<boolean> {
  const { data: o } = await admin.from("orders").select("id, status, is_draft, origin_type").eq("id", orderId).eq("tenant_id", tenantId).maybeSingle();
  if (!o || o.origin_type !== "self_service" || (o.status !== "draft" && !o.is_draft)) return false;
  const { data: its } = await admin.from("order_items").select("skip_kds").eq("order_id", orderId).eq("tenant_id", tenantId).neq("status", "cancelled");
  const allSkip = (its ?? []).length > 0 && (its ?? []).every((i: { skip_kds: boolean | null }) => i.skip_kds === true);
  const { error } = await admin.from("orders").update({ status: allSkip ? "ready" : "new", is_draft: false, updated_at: new Date().toISOString() }).eq("id", orderId).eq("tenant_id", tenantId).eq("is_draft", true);
  if (error) { log("ERROR", "release_held", "Falha ao liberar pedido segurado", { order_id: orderId, error: error.message }); return false; }
  runStockInBackground(deductStockForSkipKdsItems(admin, tenantId, orderId).catch((e) => log("WARN", "stock", "baixa de estoque falhou", { order_id: orderId, error: String(e) })));
  log("INFO", "release_held", "Pedido do autoatendimento liberado para a cozinha", { order_id: orderId, tenant_id: tenantId });
  return true;
}
  ...
  try {
    ...
    if (action === "close_cash_register") {
      ...
      try {
        ...
        const { data: cashPayments } = await admin.from("payments").select("id, amount, payment_methods(type), orders(status)").eq("cash_register_id", cash_register_id).eq("is_refunded", false);
        // Considera apenas pagamentos em dinheiro de pedidos validos (nao cancelados/draft).
        type CashPay = { id: string; amount: number; payment_methods: { type: string } | null; orders: { status: string } | null };
        ...
        // Cada linha de payments grava a parte real daquele pedido naquele metodo (fix
        // 2026-07-17), entao somar p.amount das linhas em dinheiro da a especie que
        // entrou na gaveta — sem contar cartao de pedido com split nem inflar parcial.
        const cashTotal = cashOnly.reduce((s: number, p: CashPay) => s + Number(p.amount ?? 0), 0);
    ...
    if (action === "create_order") {
      ...
      // imprimir) até o caixa receber — record_payment libera. Pix pago nunca é segurado.
      const holdUntilPaid = body.hold_until_paid === true && mappedOrigin === "self_service" && !isValidUuid(body.paid_pix_payment_id);
    ...
    if (action === "release_held_order") {
      const { order_id } = body;
      if (!isValidUuid(order_id)) return new Response(JSON.stringify({ error: "order_id is required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const { data: o } = await admin.from("orders").select("id, total_amount, is_paid").eq("id", order_id).eq("tenant_id", tenantId).maybeSingle();
      if (!o) return new Response(JSON.stringify({ error: "Pedido nao encontrado" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const { data: pays } = await admin.from("payments").select("amount").eq("order_id", order_id).eq("tenant_id", tenantId).eq("is_refunded", false);
      const pago = (pays ?? []).reduce((acc: number, p: { amount: number }) => acc + Number(p.amount ?? 0), 0);
      if (!o.is_paid && pago < Number(o.total_amount ?? 0) - 0.01) return new Response(JSON.stringify({ error: "Pedido ainda não foi pago — receba antes de mandar pra cozinha.", code: "not_paid" }), { status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      const released = await releaseHeldSelfServiceOrder(admin, tenantId, order_id);
      return new Response(JSON.stringify({ data: { released } }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "mark_order_paid") {
      ...
      try { await releaseHeldSelfServiceOrder(admin, orderTenantId2, order_id); } catch (e) { log("ERROR", "release_held", "Excecao ao liberar pedido (mark_order_paid)", { order_id, error: String(e) }); }
      return new Response(JSON.stringify({ ok: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "record_payment") {
      ...
      // Pedido do tablet segurado (dinheiro no caixa): quitou → vai pra cozinha. Antes da NFC-e,
      // que ignora rascunho.
      let released = false;
      if (becamePaid && order_id) { try { released = await releaseHeldSelfServiceOrder(admin, paymentTenantId, order_id); } catch (e) { log("ERROR", "release_held", "Excecao ao liberar pedido", { order_id, error: String(e) }); } }
      ...
      if (orderIsTraining) { log("INFO", "record_payment", "Pedido de treino — efeitos financeiros e fidelidade pulados", { order_id, payment_id: paymentId }); return new Response(JSON.stringify({ data: { id: paymentId, released }, training: true }), { headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
      ...
      return new Response(JSON.stringify({ data: { id: paymentId, released } }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
