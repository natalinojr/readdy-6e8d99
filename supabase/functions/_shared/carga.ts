/**
 * Carga de trabalho: quanto de trabalho ESTIMADO cada pessoa tem por dia.
 *
 * Mora em _shared porque a tela (Tarefas › Carga) e a Edge do aviso de
 * sobrecarga (task-sobrecarga) precisam da MESMA conta. Sem imports: só TS puro.
 *
 * Cada dia mostra o TOTAL planejado (inclusive tarefas já concluídas — pedido do
 * dono 2026-09-23) e quanto desse total já está FEITO:
 * - concluída: a estimativa inteira conta como feita, nos dias em que estava planejada;
 * - aberta: a estimativa inteira conta no total; o que já foi cronometrado é a parte feita.
 * Espalha entre início e prazo PROPORCIONAL às horas de cada dia (sábado de meio
 * período recebe metade de um dia cheio; folga/ausência marcada no dia = 0);
 * tarefa aberta atrasada não some — cai em "hoje", marcada; sem estimativa / sem
 * prazo aparecem como pendências à parte.
 */

/** Horas por dia da semana, índice 0 = domingo (igual a Date.getDay()). */
export type Capacidade = [number, number, number, number, number, number, number];

export const CAPACIDADE_PADRAO: Capacidade = [0, 8, 8, 8, 8, 8, 0];

export const SEM_RESPONSAVEL = '__sem_responsavel';

/** O mínimo que a conta precisa de uma tarefa (a tela passa TaskRow inteiro). */
export interface TarefaCarga {
  id: string;
  status_category: string | null;
  time_estimate_minutes?: number | null;
  time_tracked_seconds?: number | null;
  time_plan?: { dias?: Record<string, number> } | null;
  start_date: string | null;
  due_date: string | null;
  assignee_id: string | null;
  assignees?: Array<{ id: string }> | null;
}

export interface Parcela<T extends TarefaCarga = TarefaCarga> {
  task: T;
  /** Minutos planejados pra esse dia. */
  minutos: number;
  /** Quanto desses minutos já está feito (concluída = tudo; aberta = proporção cronometrada). */
  feitos: number;
  atrasada: boolean;
  concluida: boolean;
}

export interface ResultadoCarga<T extends TarefaCarga = TarefaCarga> {
  /** pessoa → dia (YYYY-MM-DD) → parcelas do dia */
  porPessoa: Map<string, Map<string, Parcela<T>[]>>;
  semEstimativa: T[];
  semData: T[];
  atrasadas: T[];
}

/** Folga/ausência num dia: horas disponíveis naquele dia (0 = não trabalha). */
export type ExcecaoDe = (pessoa: string, dia: string) => number | undefined;

export function chaveDia(d: Date): string {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

/** 'YYYY-MM-DD' (date) ou ISO com hora (due_date ao meio-dia UTC) → dia local. */
export function diaLocal(valor: string): Date {
  if (/^\d{4}-\d{2}-\d{2}$/.test(valor)) {
    const [a, m, d] = valor.split('-').map(Number);
    return new Date(a, m - 1, d);
  }
  const dt = new Date(valor);
  return new Date(dt.getFullYear(), dt.getMonth(), dt.getDate());
}

export function somarDias(d: Date, n: number): Date {
  const r = new Date(d);
  r.setDate(r.getDate() + n);
  return r;
}

/** Responsáveis da tarefa (o principal primeiro); tarefa antiga sem lista cai no principal. */
export function idsDaTarefa(t: Pick<TarefaCarga, 'assignee_id' | 'assignees'>): string[] {
  if (t.assignees && t.assignees.length) return t.assignees.map((a) => a.id);
  return t.assignee_id ? [t.assignee_id] : [];
}

/** Horas que a pessoa trabalha num dia: a folga/ausência do dia manda; senão o dia da semana. */
export function horasNoDia(pessoa: string, d: Date, cap: Capacidade, excecaoDe?: ExcecaoDe): number {
  const excecao = excecaoDe?.(pessoa, chaveDia(d));
  return excecao !== undefined ? excecao : cap[d.getDay()];
}

export function minutosRestantes(task: Pick<TarefaCarga, 'time_estimate_minutes' | 'time_tracked_seconds'>): number {
  if (!task.time_estimate_minutes) return 0;
  const feitos = Math.floor((task.time_tracked_seconds ?? 0) / 60);
  return Math.max(0, task.time_estimate_minutes - feitos);
}

export function calcularCarga<T extends TarefaCarga>(
  tasks: T[],
  hoje: Date,
  capacidadeDe: (pessoa: string) => Capacidade,
  excecaoDe?: ExcecaoDe,
): ResultadoCarga<T> {
  const porPessoa = new Map<string, Map<string, Parcela<T>[]>>();
  const semEstimativa: T[] = [];
  const semData: T[] = [];
  const atrasadas: T[] = [];
  const hoje0 = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());

  for (const t of tasks) {
    if (t.status_category === 'cancelled') continue;
    const concluida = t.status_category === 'done';
    const estimativa = t.time_estimate_minutes ?? 0;
    if (!estimativa) { if (!concluida) semEstimativa.push(t); continue; }
    if (!t.due_date) { if (!concluida) semData.push(t); continue; }

    // Vários responsáveis: o tempo é dividido igualmente entre eles.
    const pessoasDaTarefa = idsDaTarefa(t);
    const pessoas = pessoasDaTarefa.length ? pessoasDaTarefa : [SEM_RESPONSAVEL];
    const fatia = 1 / pessoas.length;
    const prazo = diaLocal(t.due_date);
    const atrasada = !concluida && prazo < hoje0;
    if (atrasada) atrasadas.push(t);
    // Fração já feita: concluída = 1; aberta = cronometrado ÷ estimado (até 1).
    const fracaoFeita = concluida ? 1 : Math.min(1, (estimativa - minutosRestantes(t)) / estimativa);

    for (const pessoa of pessoas) {
      const mapa = porPessoa.get(pessoa) ?? new Map<string, Parcela<T>[]>();
      porPessoa.set(pessoa, mapa);

      // Plano por dia definido na tarefa: usa exatamente os minutos de cada dia.
      const plano = t.time_plan?.dias ? Object.entries(t.time_plan.dias).filter(([, m]) => m > 0) : [];
      if (plano.length) {
        for (const [dia, total] of plano) {
          const minutos = total * fatia;
          const lista = mapa.get(dia) ?? [];
          lista.push({ task: t, minutos, feitos: minutos * fracaoFeita, atrasada, concluida });
          mapa.set(dia, lista);
        }
        continue;
      }

      let inicio = t.start_date ? diaLocal(t.start_date) : prazo;
      if (inicio > prazo) inicio = prazo;
      let fim = prazo;
      // Concluída fica onde estava planejada (é histórico). Aberta não planeja o
      // passado: começa hoje, e a atrasada cai inteira em hoje.
      if (!concluida) {
        if (inicio < hoje0) inicio = hoje0;
        if (fim < hoje0) fim = hoje0;
      }

      const cap = capacidadeDe(pessoa);
      const dias: Array<{ chave: string; peso: number }> = [];
      for (let d = inicio; d <= fim; d = somarDias(d, 1)) {
        dias.push({ chave: chaveDia(d), peso: horasNoDia(pessoa, d, cap, excecaoDe) });
      }
      let pesoTotal = dias.reduce((acc, d) => acc + d.peso, 0);
      // Nenhum dia útil no intervalo (ex.: prazo num domingo de folga): divide igual.
      if (pesoTotal === 0) { dias.forEach((d) => { d.peso = 1; }); pesoTotal = dias.length; }

      for (const d of dias) {
        if (d.peso === 0) continue;
        const minutos = (estimativa * fatia * d.peso) / pesoTotal;
        const lista = mapa.get(d.chave) ?? [];
        lista.push({ task: t, minutos, feitos: minutos * fracaoFeita, atrasada, concluida });
        mapa.set(d.chave, lista);
      }
    }
  }
  return { porPessoa, semEstimativa, semData, atrasadas };
}

export function minutosNoDia(r: ResultadoCarga<TarefaCarga>, pessoa: string, dia: string): number {
  return (r.porPessoa.get(pessoa)?.get(dia) ?? []).reduce((s, p) => s + p.minutos, 0);
}

export function feitosNoDia(r: ResultadoCarga<TarefaCarga>, pessoa: string, dia: string): number {
  return (r.porPessoa.get(pessoa)?.get(dia) ?? []).reduce((s, p) => s + p.feitos, 0);
}

/**
 * Ocupação do que FALTA de hoje até `ate` (inclusive): minutos em aberto ÷
 * minutos disponíveis. É a régua do aviso de sobrecarga e do "acima da capacidade".
 */
export function ocupacaoRestante(
  r: ResultadoCarga<TarefaCarga>,
  pessoa: string,
  hoje: Date,
  ate: Date,
  cap: Capacidade,
  excecaoDe?: ExcecaoDe,
): { faltam: number; disponivel: number } {
  let faltam = 0;
  let disponivel = 0;
  for (let d = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate()); d <= ate; d = somarDias(d, 1)) {
    const chave = chaveDia(d);
    faltam += minutosNoDia(r, pessoa, chave) - feitosNoDia(r, pessoa, chave);
    disponivel += horasNoDia(pessoa, d, cap, excecaoDe) * 60;
  }
  return { faltam: Math.max(0, faltam), disponivel };
}
