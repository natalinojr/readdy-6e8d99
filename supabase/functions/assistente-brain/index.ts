...
- set_revenue_sources (S): sources[] ⊂ 'orders','stone' (= cartão da maquininha, nome histórico),'pix','ifood','cash' (vendas em dinheiro do PDV/totem; sem efeito se 'orders' estiver ligado),'manual' — o que conta como receita recebida (Receitas › Fontes).
...
- edit_inventory: session_id (id da contagem), items[{ingredient_id, qtd_contada}] (valor FINAL correto na unidade do estoque), motivo?. Corrige uma contagem já confirmada: o estoque atual recebe só a diferença e o ajuste é corrigido na data da contagem. Item recontado numa contagem mais nova volta em bloqueados (edite na mais nova). Seguro repetir.
- set_count_inventory: ingredient_id, count_inventory (boolean). false = o insumo sai da contagem de inventário (tela e contagem rápida) e a confirmação do inventário o ignora; compra, entrada, saída, ficha técnica e CMV seguem normais. upsert_ingredient também aceita count_inventory, e count_unit + count_factor = unidade em que a equipe CONTA no inventário e quanto 1 dela vale na unidade do estoque (barbacoa em kg contada em pacote de 500 g → count_unit 'pacote', count_factor 0.5; null/null volta a contar na unidade do estoque). O inventário continua gravando na unidade do estoque.
