...
TOOLS.push({
  ...
  input_schema: {
    ...
    properties: {
      ...
      chave_pix: { type: 'string', description: 'Pix: chave do fornecedor (CNPJ, e-mail, telefone +55 ou aleatória). Só fornecedor já cadastrado pelo Natalino no ERPOS; você não cadastra fornecedor.' },
...
async function runTool(ctx: Ctx, name: string, input: any): Promise<string> {
  ...
  switch (name) {
    case 'erpos_executar': {
      ...
      if (/supplier|fornecedor/i.test(action) || /"(pix_key|chave_pix|pixkey)"/.test(dadosTxt) || (funcao === 'financial-write' && /"cnpj"/.test(dadosTxt))) {
