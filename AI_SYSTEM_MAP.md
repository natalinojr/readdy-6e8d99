...
**Fase 2 — cobrar cartão na maquininha pelo Caixa.** Migration `mp_point_pdv_terminal`: `fin_payment_provider_config.pdv_terminal_id` (a maquininha do balcão). Sem ela o caixa cairia no `terminal_id` padrão, que é a máquina ao lado de um tablet. `pix-payment`: `terminalForCaller` ganhou `station` (`'pdv'` → `pdv_terminal_id ?? terminal_id`), `create_card_charge` aceita `station`/`order_id`/`order_number` e manda o **número do pedido como `external_reference`** (é o que deixa a conciliação exata), `kiosk_card_provider` devolve `pdv`, `get/save_point_config` tratam `pdv_terminal_id`.
