...
// Tarefas › Carga (calcularCarga) e a mesma fonte de horas (fn_get_task_capacities). Só conta
// tarefas que eu criei ou sou responsável — é o alcance de fn_get_tasks, igual ao resto do módulo.
...
interface Linha { id: string; nome: string; minutos: number; capHoras: number; folga: boolean }
...
export default function CargaEquipe({ onFechar, irPara }: AcaoProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const ids = [...new Set(todas.map((t) => t.assignee_id).filter((id): id is string => !!id))];
      let capacidades: Record<string, Capacidade> = {};
      if (ids.length) {
        const { data } = await supabase.rpc('fn_get_task_capacities', { p_user_ids: ids });
      ...
      const ls: Linha[] = ids.map((id) => {
        ...
        const minutos = (resultado.porPessoa.get(id)?.get(hojeChave) ?? []).reduce((s, p) => s + p.minutos, 0);
        return { id, nome: nomeDe(id), minutos, capHoras, folga: capHoras === 0 };
      }).sort((a, b) => {
      ...
      if (!ls.length) { r.bot('Nenhuma tarefa aberta com responsável ainda.'); setPasso('fim'); return; }
      ...
      r.bot('Conta só tarefas que você criou ou é responsável.');
      setPasso('painel');
    })();
        ...
        <>
          ...
          {parcelasDoDia.map((p) => (
            <OpcaoTarefa key={p.task.id} t={p.task} onClick={() => irPara(`/tarefas?task=${p.task.id}`)}
              extra={`${formatarDuracao(p.minutos * 60)}${p.atrasada ? ' · atrasada' : ''}`} />
          ))}
          ...
          <div key={l.id}>
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={`text-xs font-medium ${cor.texto}`}>
                {l.folga && l.minutos <= 0 ? 'folga' : `${formatarHoras(l.minutos)} de ${l.capHoras}h`}
                {sobrecarregado && ' · sobrecarregado'}
                {l.folga && l.minutos > 0 && ' · folga com carga'}
              </span>
            </div>
            <div className="h-1.5 rounded-full bg-zinc-100 overflow-hidden mt-1">
