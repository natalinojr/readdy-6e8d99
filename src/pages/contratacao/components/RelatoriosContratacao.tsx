// Relatórios da contratação: funil, por loja, cargos, bairros, entradas por semana,
// comparecimento e ficha das entrevistas, ranking dos mais bem avaliados.
import { useMemo } from 'react';
import {
  type Candidate, type Interview, type Loja, STATUS, CRITERIA, RECOMMENDATIONS, avgScore, lojaNome, norm, statusInfo,
} from '../shared';
...
export default function RelatoriosContratacao({ candidates, interviews, lojas, mostrarLoja, onOpen }: Props) {
  const ids = useMemo(() => new Set(candidates.map((c) => c.id)), [candidates]);
  const ivs = useMemo(() => interviews.filter((iv) => ids.has(iv.candidate_id)), [interviews, ids]);
  ...
  const r = useMemo(() => {
    ...
    const porStatus = STATUS.map((s) => ({ ...s, count: candidates.filter((c) => c.status === s.id).length }));
    ...
    const area = {
      sim: candidates.filter((c) => c.food_service_experience === true).length,
      nao: candidates.filter((c) => c.food_service_experience === false).length,
      ind: candidates.filter((c) => c.food_service_experience == null).length,
    };

    const realizadasList = ivs.filter((i) => i.status === 'realizada');
    const criterios = CRITERIA.map((cr) => {
      const vals = realizadasList.map((i) => i.scores?.[cr.id]).filter((v): v is number => typeof v === 'number' && v > 0);
      return { ...cr, media: vals.length ? vals.reduce((s, v) => s + v, 0) / vals.length : null, n: vals.length };
    });
    const recs = RECOMMENDATIONS.map((rc) => ({ ...rc, count: realizadasList.filter((i) => i.recommendation === rc.id).length }));

    // Ranking: melhor média da ficha (última entrevista realizada) ou estrelas.
    ...
    const ranking = candidates
      .filter((c) => c.status !== 'descartado' && c.status !== 'contratado')
    ...
    const porLoja = mostrarLoja
      ? [...new Set(candidates.map((c) => c.tenant_id))].map((tid) => ({
          nome: lojaNome(lojas, tid),
          counts: STATUS.map((s) => candidates.filter((c) => c.tenant_id === tid && c.status === s.id).length),
          total: candidates.filter((c) => c.tenant_id === tid).length,
        })).sort((a, b) => b.total - a.total)
      : [];

    return {
      total, novos7, porStatus, realizadas, faltas, agendadasFuturas, comparecimento, contratados, semanas, area,
      criterios, recs, ranking, porLoja,
      cargos: topCounts(candidates.map((c) => c.desired_role)),
      locais: topCounts(candidates.map((c) => c.neighborhood || c.city)),
    };
  }, [candidates, ivs, lojas, mostrarLoja]);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Kpi label="Contratados" value={r.contratados} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card titulo="Funil por etapa">
          <div className="space-y-2">
            {r.porStatus.map((s) => (
              <Bar key={s.id} label={s.label} count={s.count} total={r.total} cls={s.bar} />
            ))}
          </div>
        </Card>
        ...
        <Card titulo="Experiência em restaurante">
          <div className="space-y-2">
            <Bar label="Já trabalhou na área" count={r.area.sim} total={r.total} cls="bg-emerald-500" />
            <Bar label="Não trabalhou na área" count={r.area.nao} total={r.total} cls="bg-zinc-400" />
            <Bar label="Sem informação" count={r.area.ind} total={r.total} cls="bg-zinc-200" />
          </div>
          <p className="text-[11px] text-zinc-400 mt-2">"Sem informação" diminui ao organizar os currículos com IA.</p>
        </Card>
      ...
      {r.porLoja.length > 1 && (
        <Card titulo="Por loja">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-[10px] uppercase tracking-wider text-zinc-400">
                <th className="text-left py-1.5 pr-3">Loja</th>
                {STATUS.map((s) => <th key={s.id} className="text-right py-1.5 px-2">{s.label}</th>)}
      ...
      <Card titulo="Mais bem avaliados (ainda no processo)">
              ...
              <li key={c.id}>
                <button onClick={() => onOpen(c.id)} className="w-full flex items-center gap-3 py-2 text-left hover:bg-zinc-50 rounded-lg px-2 cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-zinc-500 truncate">{[c.desired_role, mostrarLoja ? lojaNome(lojas, c.tenant_id) : null].filter(Boolean).join(' · ')}</p>
                  </div>
                  {rec && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${RECOMMENDATIONS.find((x) => x.id === rec)?.cls}`}>{RECOMMENDATIONS.find((x) => x.id === rec)?.label}</span>}
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusInfo(c.status).cls}`}>{statusInfo(c.status).label}</span>
