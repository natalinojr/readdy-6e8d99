...
interface DREData {
  ...
  receitaStone: number;
  /** Vendas em cartão abertas por maquininha (Stone, Mercado Pago) — sublinhas, soma = receitaStone. */
  cartaoPorMaquininha?: ReceitaDetalhe;
  /** Pix que entrou no Inter — só com a fonte "pix" ligada (fin_revenue_settings). */
  receitaPix?: number;
  /** Pix recebido aberto pela etiqueta da Conciliação (Tuna, vouchers, tablet...) — sublinhas, soma = receitaPix. */
...
async function fetchDREDataCompetencia(tenantId: string, startDate: string, endDate: string, temPdv: boolean): Promise<DREData> {
  ...
  // Mercado Pago segue pela data da liberação (não tem arquivo por data da venda)
...
export default function DRETab() {
  ...
  const prevReceitaRecebida = (prevData?.receitaBalcao ?? 0) + (prevData?.receitaDelivery ?? 0)
    + (prevData?.receitaMesa ?? 0) + (prevData?.receitaAutoatendimento ?? 0) + (prevData?.receitaManual ?? 0) + (prevData?.receitaStone ?? 0) + (prevData?.receitaPix ?? 0) + (prevData?.receitaIfood ?? 0) + (prevData?.receitaDinheiro ?? 0);
