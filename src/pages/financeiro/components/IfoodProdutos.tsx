  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-zinc-100 p-4"><p className="text-xs text-zinc-500">Itens vendidos</p><p className="text-xl font-bold text-zinc-900 mt-1">{qtdItens.toLocaleString('pt-BR')}</p><p className="text-[11px] text-zinc-400">{itens.length} produto(s) diferente(s)</p></div>
        <div className="bg-white rounded-xl border border-zinc-100 p-4"><p className="text-xs text-zinc-500">Valor dos itens</p><p className="text-xl font-bold text-zinc-900 mt-1">{formatCurrency(totItens)}</p><p className="text-[11px] text-zinc-400">preço médio {formatCurrency(qtdItens ? totItens / qtdItens : 0)}</p></div>
        <div className="bg-white rounded-xl border border-zinc-100 p-4"><p className="text-xs text-zinc-500">Complementos</p><p className="text-xl font-bold text-zinc-900 mt-1">{comps.reduce((s, c) => s + c.qtd, 0).toLocaleString('pt-BR')}</p><p className="text-[11px] text-zinc-400">{formatCurrency(comps.reduce((s, c) => s + c.valor, 0))} cobrados à parte</p></div>
      </div>
            ...
            <tbody>
              {itens.map((i) => (
                <tr key={i.name} className="border-t border-zinc-100">
                  ...
                  <td className="px-3 py-2 text-right font-mono">{i.qtd.toLocaleString('pt-BR')}</td>
                  <td className="px-3 py-2 text-right font-mono text-zinc-500">{i.pedidos.toLocaleString('pt-BR')}</td>
                  <td className="px-3 py-2 text-right font-mono">{formatCurrency(i.valor)}</td>
                  <td className="px-3 py-2 text-right font-mono text-zinc-500">{formatCurrency(i.qtd ? i.valor / i.qtd : 0)}</td>
            ...
            <tbody>
              {comps.map((c) => (
                <tr key={c.name} className="border-t border-zinc-100">
                  ...
                  <td className="px-3 py-2 text-right font-mono">{c.qtd.toLocaleString('pt-BR')}</td>
                  <td className="px-3 py-2 text-right font-mono text-zinc-500">{c.pedidos.toLocaleString('pt-BR')}</td>
                  <td className="px-3 py-2 text-right font-mono">{c.valor ? formatCurrency(c.valor) : <span className="text-zinc-400">incluso</span>}</td>
                </tr>
              ))}
