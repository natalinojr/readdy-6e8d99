  ...
  return (
    <div className="fixed inset-0 bg-zinc-950 flex flex-col items-center justify-center p-8">
      <div className="text-center mb-14">
        <div className="w-24 h-24 flex items-center justify-center bg-amber-500 rounded-2xl mx-auto mb-6">
          <i className="ri-lock-line text-5xl text-zinc-950" />
        </div>
        <h2 className="text-5xl font-black text-white mb-3">Autoatendimento</h2>
        <p className="text-zinc-500 text-xl">Digite o PIN de acesso para liberar o totem</p>
      </div>

      {/* Pontos do PIN */}
      <div className={`flex items-center gap-6 mb-10 transition-all ${shake ? 'translate-x-2' : ''}`} style={{ animation: shake ? 'shake 0.4s ease' : 'none' }}>
        {Array.from({ length: maxLen }).map((_, i) => (
          <div
            key={i}
            className={`w-8 h-8 rounded-full transition-all ${i < input.length ? 'bg-amber-400 scale-110' : 'bg-zinc-700'}`}
          />
        ))}
      </div>

      {tentativas >= 3 && (
        <p className="text-red-400 text-lg font-semibold mb-6 animate-pulse">
          ...
          <div key={ri} className="flex gap-4">
            {row.map((d) => (
              <button
                ...
                className={`w-28 h-24 flex items-center justify-center rounded-2xl text-3xl font-bold cursor-pointer transition-all select-none ${
                  d === '*'
              ...
              >
                {d === '⌫' ? <i className="ri-delete-back-2-line text-4xl" /> : d}
              </button>
            ))}
          </div>
        ))}
      ...
      <p className="text-zinc-700 text-base mt-10">
