...
interface PagamentoKioskProps {
  ...
  identifNome: string;
  identifSenha: string;
  modoIdentificacao: 'nome' | 'senha' | 'comanda' | 'nenhum';
...
function TelaConfirmacao({
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-3 p-4 md:p-6 text-center overflow-y-auto">
      {/* Ícone animado */}
      <div className="relative">
        <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center bg-emerald-500/20 rounded-full">
          <i className="ri-checkbox-circle-fill text-4xl md:text-5xl text-emerald-400" />
        </div>
      ...
      <div className="flex gap-2 md:gap-3 w-full max-w-2xl">
        {modoIdentificacao !== 'nenhum' && (
          <div className="bg-amber-500/10 border-2 border-amber-500/40 rounded-xl px-4 py-3 flex flex-col items-center justify-center flex-shrink-0">
            <p className="text-amber-400 text-xs font-bold uppercase tracking-widest mb-1">{identificadorLabel}</p>
            <p className={`text-amber-400 font-black leading-none ${isNome ? 'text-lg md:text-2xl' : 'text-4xl md:text-6xl tracking-wider'}`}>
              {identificadorValor}
            </p>
          </div>
        )}
        <div className="bg-zinc-800 rounded-xl p-2.5 flex-1 overflow-y-auto max-h-32 md:max-h-40">
          {carrinho.map((item, i) => (
            <div key={i} className="flex justify-between text-xs md:text-sm py-0.5">
          ...
          <div className="border-t border-zinc-700 mt-1.5 pt-1.5 flex justify-between">
            <span className="text-white font-bold text-xs md:text-sm">{pago ? 'Total pago' : 'Total a pagar'}</span>
            <span className="text-amber-400 font-black text-sm md:text-base">{fmt(total)}</span>
          </div>
        </div>
      </div>

      {!pago && (
        <div className="bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 max-w-2xl w-full text-center">
          <div className="flex items-center justify-center gap-2 text-amber-400 mb-1">
            <i className="ri-information-line text-sm flex-shrink-0" />
            <p className="text-xs md:text-sm font-semibold">Pague no balcão ao retirar o pedido</p>
          </div>
          {formaPagamentoNome ? (
            <div className="flex items-center justify-center gap-2">
              <i className="ri-wallet-3-line text-emerald-400 text-sm" />
              <p className="text-xs md:text-sm font-bold text-emerald-400">Forma: {formaPagamentoNome}</p>
            </div>
          ) : (
            <p className="text-xs text-zinc-500">Aceitamos: Dinheiro, PIX, Cartão</p>
          )}
        </div>
      )}

      {alertaParcial && (
        <div className="w-full max-w-2xl bg-amber-900/40 border border-amber-500/50 rounded-xl px-4 py-3 flex items-start gap-3">
          <i className="ri-alert-line text-amber-400 text-sm flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-amber-300 font-bold text-xs">Aviso para o operador</p>
      ...
      <div className="flex flex-col items-center gap-2">
        <button
          ...
          className="px-8 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white font-bold text-sm md:text-base rounded-xl cursor-pointer transition-colors whitespace-nowrap"
        >
          Novo pedido
        </button>
        ...
        <div className="w-32 md:w-40 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
          <div
...
function TelaPix({
  ...
  const handleConfirmarManual = async () => {
    ...
    try {
      await supabase.functions.invoke('pix-payment', {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        <div className="w-16 h-16 flex items-center justify-center bg-emerald-500/10 rounded-2xl">
          <div className="w-8 h-8 border-3 border-emerald-500 border-t-transparent rounded-full animate-spin" style={{ borderWidth: 3 }} />
        </div>
        <p className="text-zinc-400 text-base font-semibold">Gerando QR Code PIX...</p>
      </div>
    );
  }
  ...
  if (pixError) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
        <div className="w-16 h-16 flex items-center justify-center bg-red-500/10 rounded-2xl">
          <i className="ri-error-warning-line text-3xl text-red-400" />
        </div>
        <div>
          <h3 className="text-lg font-bold text-white mb-1">PIX não configurado</h3>
          <p className="text-zinc-400 text-sm max-w-xs">{pixError}</p>
        </div>
        <div className="flex gap-3">
          <button onClick={onVoltar} className="px-5 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white font-semibold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap">
            Escolher outra forma
          </button>
          <button onClick={generatePix} className="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap">
            Tentar novamente
          </button>
        </div>
      </div>
  ...
  if (pollingStatus === 'confirmed') {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
        <div className="relative">
          <div className="w-20 h-20 flex items-center justify-center bg-emerald-500/20 rounded-full">
            <i className="ri-checkbox-circle-fill text-5xl text-emerald-400" />
          </div>
          <div className="absolute inset-0 rounded-full border-4 border-emerald-500/30 animate-ping" />
        </div>
        <div>
          <h3 className="text-2xl md:text-3xl font-black text-white">PIX Confirmado!</h3>
          <p className="text-emerald-400 font-semibold text-sm mt-1">Pagamento recebido com sucesso</p>
        </div>
        ...
        <div className="flex gap-2 items-center bg-zinc-800 px-4 py-2 rounded-xl">
          ...
          <p className="text-zinc-300 text-sm">Registrando pedido...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
        <div className="w-16 h-16 flex items-center justify-center bg-amber-500/10 rounded-2xl">
          <i className="ri-time-line text-3xl text-amber-400" />
        </div>
        <div>
          <h3 className="text-xl font-bold text-white mb-1">QR Code expirado</h3>
          <p className="text-zinc-400 text-sm">O tempo para pagamento esgotou. Gere um novo QR Code.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={onVoltar} className="px-5 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white font-semibold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap">
            Voltar
          </button>
          <button onClick={generatePix} className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap">
            Gerar novo QR Code
          </button>
        </div>
      </div>
  ...
  // ── Tela principal PIX ──
  return (
    <div className="flex flex-col items-center justify-center h-full gap-3 md:gap-4 p-4 md:p-6 text-center overflow-y-auto">
      {/* Header */}
      <div>
        <div className="flex items-center justify-center gap-2 mb-1">
          <div className="w-6 h-6 flex items-center justify-center bg-emerald-500/20 rounded-lg">
            <i className="ri-qr-code-line text-emerald-400 text-sm" />
          </div>
          <h2 className="text-xl md:text-3xl font-black text-white">Pague com PIX</h2>
        </div>
        <p className="text-zinc-400 text-sm">Abra o app do seu banco e escaneie o QR Code</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 w-full max-w-2xl">
        ...
        <div className="flex flex-col gap-2 w-full md:flex-1">
          ...
          <div className="bg-zinc-800 rounded-xl px-4 py-3 text-center">
            <p className="text-zinc-500 text-xs mb-0.5">Valor a pagar</p>
          ...
          {pixData?.pix_key && (
            <div className="bg-zinc-800 rounded-xl px-4 py-3">
              ...
              <p className="text-white font-mono text-xs md:text-sm font-bold break-all">{pixData.pix_key}</p>
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-semibold cursor-pointer transition-all whitespace-nowrap ${
              copiado
                ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40'
                : 'bg-zinc-700 hover:bg-zinc-600 text-zinc-300'
            }`}
          >
            <i className={`text-sm ${copiado ? 'ri-checkbox-circle-line' : 'ri-file-copy-line'}`} />
      ...
      <div className="w-full max-w-2xl">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`text-sm font-black ${timeLeft < 60 ? 'text-red-400' : timeLeft < 180 ? 'text-amber-400' : 'text-zinc-300'}`}>
            {mins}:{secs}
          </span>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full max-w-2xl">
        <button
          onClick={onVoltar}
          className="flex-1 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-semibold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap"
        >
          Escolher outra forma
        </button>
        <button
          ...
          className="flex-1 py-2.5 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-400 font-bold text-sm rounded-xl cursor-pointer transition-colors whitespace-nowrap disabled:opacity-60"
        >
...
export default function PagamentoKiosk({
  ...
  const isComanda = modoIdentificacao === 'comanda';
  const isNome = modoIdentificacao === 'nome';
  const identificadorLabel = isComanda ? 'Comanda Nº' : isNome ? 'Seu nome' : modoIdentificacao === 'senha' ? 'Sua senha' : 'Pedido';
  ...
  if (processandoPedido) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-8 text-center">
        ...
        <p className="text-zinc-400 text-base md:text-xl">Registrando seu pedido...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 md:gap-6 p-5 md:p-8">
        <div className="text-center">
          <h2 className="text-xl md:text-3xl font-black text-white mb-1">Quando deseja pagar?</h2>
          <p className="text-zinc-400 text-sm md:text-base">Total: <span className="text-amber-400 font-black">{fmt(total)}</span></p>
        </div>
        <div className="grid grid-cols-2 gap-3 md:gap-4 w-full max-w-xl">
          <button
            onClick={() => setModoEscolhido('hora')}
            className="flex flex-col items-center gap-3 md:gap-4 p-4 md:p-6 bg-amber-500 hover:bg-amber-400 text-zinc-950 rounded-2xl md:rounded-3xl cursor-pointer active:scale-95 transition-all"
          >
            <div className="w-11 h-11 md:w-14 md:h-14 flex items-center justify-center bg-zinc-950/10 rounded-xl md:rounded-2xl">
              <i className="ri-secure-payment-line text-2xl md:text-3xl" />
            </div>
            <div className="text-center">
              <p className="text-base md:text-xl font-black">Pagar agora</p>
              <p className="text-zinc-950/60 text-xs md:text-sm mt-0.5">PIX, cartão ou dinheiro</p>
            </div>
          </button>
          <button
            onClick={handlePagarNaEntregaEscolhido}
            className="flex flex-col items-center gap-3 md:gap-4 p-4 md:p-6 bg-zinc-800 hover:bg-zinc-700 text-white rounded-2xl md:rounded-3xl cursor-pointer active:scale-95 transition-all"
          >
            <div className="w-11 h-11 md:w-14 md:h-14 flex items-center justify-center bg-zinc-700 rounded-xl md:rounded-2xl">
              <i className="ri-store-2-line text-2xl md:text-3xl text-zinc-300" />
            </div>
            <div className="text-center">
              <p className="text-base md:text-xl font-black">Pagar na entrega</p>
              <p className="text-zinc-500 text-xs md:text-sm mt-0.5">Pague ao retirar no balcão</p>
            </div>
          </button>
        </div>
      </div>
  ...
  if (!forma) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 md:gap-5 p-5 md:p-8">
        <div className="text-center">
          <h2 className="text-xl md:text-3xl font-black text-white mb-1">Como deseja pagar?</h2>
          <p className="text-zinc-400 text-sm md:text-base">Total: <span className="text-amber-400 font-black">{fmt(total)}</span></p>
        </div>
        <div className="grid grid-cols-2 gap-2 md:gap-3 w-full max-w-xl">
          {paymentMethods.map((method) => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-xl ${isPix ? 'bg-white/20' : 'bg-zinc-700'}`}>
                  <i className={`${getMethodIcon(method.type)} text-xl md:text-2xl`} />
                </div>
                <div className="text-center">
                  <p className="text-sm md:text-lg font-black">{method.name}</p>
  ...
  // ── Cartão / Dinheiro ──
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 md:gap-5 p-5 md:p-8 text-center">
      <div className="w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-zinc-800 rounded-xl md:rounded-2xl">
        <i className={`text-2xl md:text-3xl text-amber-400 ${forma.type === 'cash' ? 'ri-money-dollar-circle-line' : 'ri-bank-card-line'}`} />
      </div>
      ...
      <p className="text-zinc-400 text-sm md:text-lg">Total: <span className="text-amber-400 font-black">{fmt(total)}</span></p>
      {forma.type !== 'cash' && (
        <div className="bg-zinc-800 rounded-xl md:rounded-2xl px-8 md:px-12 py-4 md:py-6 border-2 border-dashed border-zinc-600">
          <p className="text-zinc-500 text-sm md:text-base">Aguardando leitura do terminal...</p>
      ...
      <div className="flex gap-2 md:gap-3">
        <button
          ...
          className="px-4 md:px-6 py-2.5 md:py-3 bg-zinc-700 hover:bg-zinc-600 text-white font-bold text-sm md:text-base rounded-xl md:rounded-2xl cursor-pointer whitespace-nowrap"
        >
          Voltar
        </button>
        <button
          ...
          disabled={aguardando}
          className="px-6 md:px-8 py-2.5 md:py-3 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-60 text-white font-bold text-sm md:text-base rounded-xl md:rounded-2xl cursor-pointer whitespace-nowrap"
        >
