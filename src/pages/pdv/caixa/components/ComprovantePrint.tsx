...
function buildReceiptHTML(props: Omit<Props, 'onClose'>): string {
  const { numero, carrinho, total, desconto, destino, pagamentos, operador, loja } = props;
...
<body>
  ...
  <div style="text-align:center;font-size:10px;margin-bottom:2px;">Pedido #${numStr} · ${dataHora}</div>
...
export default function ComprovantePrint({
  numero, carrinho, total, desconto, destino, pagamentos, operador, loja, onClose,
}: Props) {
  const troco = pagamentos.reduce((acc, p) => acc + (p.troco ?? 0), 0);
  const subtotal = carrinho.reduce((acc, i) => acc + i.precoTotal * i.quantidade, 0);

  const handlePrint = () => {
    const html = buildReceiptHTML({ numero, carrinho, total, desconto, destino, pagamentos, operador, loja });
    printHTML(html);
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-zinc-900">Comprovante #{String(numero).padStart(4, '0')}</p>
        ...
        <div className="p-5 max-h-80 overflow-y-auto">
          <div className="font-mono text-xs text-zinc-800 leading-relaxed space-y-1">
            ...
            <div className="text-center text-zinc-500 text-[10px]">Pedido #{String(numero).padStart(4, '0')} · {fmtData()}</div>
